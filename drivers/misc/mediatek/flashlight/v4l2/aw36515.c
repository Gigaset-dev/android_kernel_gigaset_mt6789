...
#define AW36515_FLASH_MAX_CURRENT 1  //LED1&LED2 MAX:1A
#define AW36515_DUAL_FLASH_SAME 1
...
/*  FLASH Brightness
 *	min 391000uA, step 7830uA, max 2000000uA
 */
#define AW36515_FLASH_BRT_MIN 3910
#define AW36515_FLASH_BRT_STEP 7830
#if AW36515_FLASH_MAX_CURRENT
	#define AW36515_FLASH_BRT_MAX 1000000
#else
 ...
 */
#define AW36515_FLASH_TOUT_MIN 100
#define AW36515_FLASH_TOUT_STEP 50
#define AW36515_FLASH_TOUT_MAX 400

/*  TORCH BRT
 *	min 977uA, step 1960uA, max 500000uA
 */
...
#define AW36515_COOLER_MAX_STATE 4
...
#if AW36515_DUAL_FLASH_SAME
static const unsigned char aw36515_torch_level[AW36515_LEVEL_NUM] = {
	0x10, 0x18, 0x20, 0x28, 0x30, 0x38, 0x40, 0x00, 0x00, 0x00,
...
struct aw36515_flash {
	...
	unsigned long target_current[AW36515_LED_MAX];
...
static int aw36515_mode_ctrl(struct aw36515_flash *flash)
{
	...
	printk_ratelimited("%s mode:%d", __func__, flash->led_mode);
			      ...
			      enum aw36515_led_id led_no, bool on)
{
	...
	if (led_no < 0 || led_no >= AW36515_LED_MAX) {
		printk("led_no error\n");
		return -1;
	}
	printk_ratelimited("%s led:%d enable:%d", __func__, led_no, on);

	flashlight_kicker_pbm(on);
	if (flashlight_pt_is_low()) {
		printk_ratelimited("pt is low\n");
		return 0;
	}
#if AW36515_DUAL_FLASH_SAME	
	if(on){
				 ...
				 enum aw36515_led_id led_no, unsigned int brt)
{
	...
	if (flash->need_cooler == 0) {
		...
		if (brt > flash->target_current[led_no]) {
			...
			printk("thermal limit current:%d\n", brt);
		}
	}

	br_bits = AW36515_TORCH_BRT_uA_TO_REG(brt);
	for (i = 0; i < AW36515_LEVEL_NUM; i++) {
		if (br_bits <= aw36515_torch_level[i])
			break;
	}
	br_bits = aw36515_torch_level[i];
#if AW36515_DUAL_FLASH_SAME	
				 ...
				 enum aw36515_led_id led_no, unsigned int brt)
{
	...
	printk("%s %d brt:%u", __func__, led_no, brt);
	...
	for (i = 0; i < AW36515_LEVEL_NUM; i++) {
		if (br_bits <= aw36515_flash_level[i])
			break;
	}
	br_bits = aw36515_flash_level[i];
#if AW36515_DUAL_FLASH_SAME		
...
static int aw36515_set_ctrl(struct v4l2_ctrl *ctrl, enum aw36515_led_id led_no)
{
	...
	printk("%s led:%d ID:%d", __func__, led_no, ctrl->id);
	mutex_lock(&flash->lock);
...
static int aw36515_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		printk_ratelimited("FLASH_IOC_SET_ONOFF(%d): %d\n",
    ...
    case FLASH_IOC_SET_DUTY:
        ...
        aw36515_torch_brt_ctrl(aw36515_flash_data, channel, AW36515_TORCH_BRT_REG_TO_uA(aw36515_torch_level[(int)fl_arg->arg]));
        break;
    default:
					...
					unsigned long state)
{
	...
	printk("set thermal current:%lu\n", flash->target_state);

	if (flash->target_state == 0) {
		flash->need_cooler = 0;
		flash->target_current[AW36515_LED0] = AW36515_FLASH_BRT_MAX;
		flash->target_current[AW36515_LED1] = AW36515_FLASH_BRT_MAX;
		...
		flash->need_cooler = 1;
		flash->target_current[AW36515_LED0] =
			flash_state_to_current_limit[flash->target_state - 1];
		flash->target_current[AW36515_LED1] =
			flash_state_to_current_limit[flash->target_state - 1];
		ret = aw36515_torch_brt_ctrl(flash, AW36515_LED0,
					flash->target_current[AW36515_LED0]);
		ret = aw36515_torch_brt_ctrl(flash, AW36515_LED1,
			...
			const struct i2c_device_id *devid)
{
	...
	flash->target_current[AW36515_LED0] = AW36515_FLASH_BRT_MAX;
	flash->target_current[AW36515_LED1] = AW36515_FLASH_BRT_MAX;
	flash->ori_current[AW36515_LED0] = AW36515_TORCH_BRT_MIN;
	flash->ori_current[AW36515_LED1] = AW36515_TORCH_BRT_MIN;
	...
	if (IS_ERR(flash->cdev))
		printk("register thermal failed\n");
