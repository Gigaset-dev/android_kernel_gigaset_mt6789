...
static ssize_t manual_duty_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	...
	pr_err("gc9a01 manual_duty_store = 0x%x,0x%x,count = %d duty = %d\n",buf[0],buf[1],count_t,duty);
...
static int gc9a01_sysfs_create(void)
{
	...
	pr_err("gc9a01_sysfs_create enter !!\n");
...
static void gc9a01_set_pwm_duty(unsigned char duty)
{
	...
	printk("gc9a01_set_pwm_duty duty=%d\n",duty);
	if(duty <= 0)
	{
		mt_pwm_disable(PWM0, 0);
		__pm_relax(&gc9a01_data.pwm_wakelock);
		return;
	}
	...
	__pm_stay_awake(&gc9a01_data.pwm_wakelock);
	memset(&pwm_setting, 0, sizeof(struct pwm_spec_config));
    pwm_setting.pwm_no = PWM0;
...
int gc9a01_probe(struct spi_device *spi)
{
	...
	wakeup_source_init(&gc9a01_data.pwm_wakelock, "miniscr_wakelock");
