...
static int gc9a01_init(struct spi_device *spi){
	...
	gc9a01_write_cmd(spi,0xEB);	
	gc9a01_write_data(spi,0x14); 
	
	gc9a01_write_cmd(spi,0x84);			
	gc9a01_write_data(spi,0x60); //40->60 0xb5 en  20210529  james
	
	gc9a01_write_cmd(spi,0x88);			
	gc9a01_write_data(spi,0x0A);
	
	gc9a01_write_cmd(spi,0x89);			
	gc9a01_write_data(spi,0x23);  ///spi 2data reg en  20210529
	
	gc9a01_write_cmd(spi,0x8A);			
	gc9a01_write_data(spi,0x00); 
	
	gc9a01_write_cmd(spi,0x8B);			
	gc9a01_write_data(spi,0x80); 
	
	gc9a01_write_cmd(spi,0x8C);			
	gc9a01_write_data(spi,0x01); 
	
	gc9a01_write_cmd(spi,0x8D);			
	gc9a01_write_data(spi,0x03); 
	
	gc9a01_write_cmd(spi,0x8F);			
	gc9a01_write_data(spi,0xFF); 
	gc9a01_write_cmd(spi,0xB5);   
	gc9a01_write_data(spi,0x08);
	gc9a01_write_data(spi,0x09);//  james add 20210529
	gc9a01_write_data(spi,0x14);
	gc9a01_write_data(spi,0x08);
	
	gc9a01_write_cmd(spi,0xB6);			
	gc9a01_write_data(spi,0x00); 
	gc9a01_write_data(spi,0x60); 
	
	gc9a01_write_cmd(spi,0x36);			
	gc9a01_write_data(spi,0x48);
	
	gc9a01_write_cmd(spi,0x3A);			
	gc9a01_write_data(spi,0x05); 
	
	
	gc9a01_write_cmd(spi,0x90);			
	gc9a01_write_data(spi,0x08);
	gc9a01_write_data(spi,0x08);
	gc9a01_write_data(spi,0x08);
	gc9a01_write_data(spi,0x08); 
	
	gc9a01_write_cmd(spi,0xBA);			
	gc9a01_write_data(spi,0x01);//TE WIDTH
	
	gc9a01_write_cmd(spi,0xBD);			
	gc9a01_write_data(spi,0x06);
	
	gc9a01_write_cmd(spi,0xBC);			
	gc9a01_write_data(spi,0x00);	
	
	gc9a01_write_cmd(spi,0xFF);			
	gc9a01_write_data(spi,0x60);
	gc9a01_write_data(spi,0x01);
	gc9a01_write_data(spi,0x04);
	
	
	gc9a01_write_cmd(spi,0xC3);			
	gc9a01_write_data(spi,0x2F);
	gc9a01_write_cmd(spi,0xC4);			
	gc9a01_write_data(spi,0x2F);
	
	gc9a01_write_cmd(spi,0xC9);			
	gc9a01_write_data(spi,0x25);
	
	
	gc9a01_write_cmd(spi,0xBE);			
	gc9a01_write_data(spi,0x11); 
	
	gc9a01_write_cmd(spi,0xE1);			
	gc9a01_write_data(spi,0x10);
	gc9a01_write_data(spi,0x0E);
	
	gc9a01_write_cmd(spi,0xDF);			
	gc9a01_write_data(spi,0x21);
	gc9a01_write_data(spi,0x10);
	gc9a01_write_data(spi,0x02);
	
	gc9a01_write_cmd(spi,0xF0);   
	gc9a01_write_data(spi,0x49);
	gc9a01_write_data(spi,0x0e);
	gc9a01_write_data(spi,0x09);
	gc9a01_write_data(spi,0x09);
	gc9a01_write_data(spi,0x25);
	gc9a01_write_data(spi,0x2e);
	
	gc9a01_write_cmd(spi,0xF1);    
	gc9a01_write_data(spi,0x44);
	gc9a01_write_data(spi,0x70);
	gc9a01_write_data(spi,0x73);
	gc9a01_write_data(spi,0x2F);
	gc9a01_write_data(spi,0x30);  
	gc9a01_write_data(spi,0x6F);
	
	gc9a01_write_cmd(spi,0xF2);   
	gc9a01_write_data(spi,0x49);
	gc9a01_write_data(spi,0x0e);
	gc9a01_write_data(spi,0x09);
	gc9a01_write_data(spi,0x09);
	...
	gc9a01_write_cmd(spi,0xED);	
	gc9a01_write_data(spi,0x1B); 
	gc9a01_write_data(spi,0x8B); 
	
	gc9a01_write_cmd(spi,0xAE);			
	gc9a01_write_data(spi,0x77);
	
	gc9a01_write_cmd(spi,0xCD);			
	gc9a01_write_data(spi,0x63);		
	
	gc9a01_write_cmd(spi,0xAC);			
	gc9a01_write_data(spi,0x27);
	
	gc9a01_write_cmd(spi,0x70);			
	gc9a01_write_data(spi,0x07);
	gc9a01_write_data(spi,0x07);
	gc9a01_write_data(spi,0x04);
	gc9a01_write_data(spi,0x06);//VGH
	gc9a01_write_data(spi,0x0F); //VGL
	gc9a01_write_data(spi,0x09);
	gc9a01_write_data(spi,0x07);
	gc9a01_write_data(spi,0x08);
	gc9a01_write_data(spi,0x03);
	
	gc9a01_write_cmd(spi,0xE8);			
	gc9a01_write_data(spi,0x24);
	
	gc9a01_write_cmd(spi,0x60);		
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0x0B);
	gc9a01_write_data(spi,0x6D);
	gc9a01_write_data(spi,0x6D);
	
	gc9a01_write_data(spi,0x39);
	gc9a01_write_data(spi,0xF0);
	gc9a01_write_data(spi,0x6D);
	gc9a01_write_data(spi,0x6D);
	
	
	gc9a01_write_cmd(spi,0x61);
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0xF4);
	gc9a01_write_data(spi,0x6D);
	gc9a01_write_data(spi,0x6D);
	
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0xF7);
	gc9a01_write_data(spi,0x6D);
	gc9a01_write_data(spi,0x6D);
	/////////////////////////////////////
	gc9a01_write_cmd(spi,0x62);
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0x0D);
	gc9a01_write_data(spi,0x71);
	gc9a01_write_data(spi,0xED);
	gc9a01_write_data(spi,0x70);
	gc9a01_write_data(spi,0x70);
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0x0F);
	gc9a01_write_data(spi,0x71);
	gc9a01_write_data(spi,0xEF);
	gc9a01_write_data(spi,0x70); 
	gc9a01_write_data(spi,0x70);
	...
	gc9a01_write_data(spi,0x11);
	gc9a01_write_data(spi,0x71);
	gc9a01_write_data(spi,0xF1);
	gc9a01_write_data(spi,0x70);
	gc9a01_write_data(spi,0x70);
	gc9a01_write_data(spi,0x38);
	gc9a01_write_data(spi,0x13);
	gc9a01_write_data(spi,0x71);
	gc9a01_write_data(spi,0xF3);
	gc9a01_write_data(spi,0x70); 
	gc9a01_write_data(spi,0x70);
	
	
	gc9a01_write_cmd(spi,0x64);			
	gc9a01_write_data(spi,0x28);
	gc9a01_write_data(spi,0x29);
	gc9a01_write_data(spi,0xF1);
	gc9a01_write_data(spi,0x01);
	gc9a01_write_data(spi,0xF1);
	gc9a01_write_data(spi,0x00);//
	gc9a01_write_data(spi,0x1a);//
	
	gc9a01_write_cmd(spi,0x66);
	gc9a01_write_data(spi,0x3C);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x98);
	gc9a01_write_data(spi,0x10);
	gc9a01_write_data(spi,0x32);
	gc9a01_write_data(spi,0x45);
	gc9a01_write_data(spi,0x01);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_cmd(spi,0x67);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x3C);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x00);
	gc9a01_write_data(spi,0x10);
	gc9a01_write_data(spi,0x54);
	gc9a01_write_data(spi,0x67);
	gc9a01_write_data(spi,0x45);
	gc9a01_write_data(spi,0xcd);
	
	
	gc9a01_write_cmd(spi,0x74);			
	gc9a01_write_data(spi,0x10);	
	gc9a01_write_data(spi,0x85);	//85
	gc9a01_write_data(spi,0x80);
	gc9a01_write_data(spi,0x00); 
	gc9a01_write_data(spi,0x00); 
	gc9a01_write_data(spi,0x4E);
	gc9a01_write_data(spi,0x00);					
	
	gc9a01_write_cmd(spi,0x98);			
	gc9a01_write_data(spi,0x3e);
	gc9a01_write_data(spi,0x07);
	gc9a01_write_cmd(spi,0x99);			
	gc9a01_write_data(spi,0x3e);
	gc9a01_write_data(spi,0x07);
	
	
	gc9a01_write_cmd(spi,0x35);	
	gc9a01_write_cmd(spi,0x21);
	msleep(10);
	//msleep(10);
 ...
 // platform_delay_ms(120);
  ...
  //platform_delay_ms(20);
	...
	lcd_gc9a01_set_window(spi, 0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1, 0, 0);
	return 0;
}
