...
/* drv add for tms nfc ata test 20230802-start */
#define TMS_STATUS

#ifdef TMS_STATUS
static int tms_nfc_driver_status = 0;

static ssize_t tms_chip_status_show(struct device* dev,
                                      struct device_attribute* attr, char* buf)
{
    int len = 0;
    len += sprintf((char*)buf, "%d\n", tms_nfc_driver_status);
    return len;
}

static DEVICE_ATTR(nfc_chip_status, S_IRUGO | S_IWUSR, tms_chip_status_show, NULL);
...
#endif
/* drv add for tms nfc ata test 20230802-end */
...
/* drv add for tms nfc ata test 20230802-start */
#ifdef TMS_STATUS
        ret = sysfs_create_group(&dev->creation->kobj, &nfc_attribute_group);
	if (ret == 0)
	{
		TMS_INFO("TMS_STATUS node create success!");
		tms_nfc_driver_status = 1;
	}
	else
		TMS_INFO("TMS_STATUS node create fail!");
#endif
/* drv add for tms nfc ata test 20230802-end */
