...
static int ese_device_probe(struct spi_device *client)
{
    ...
    TMS_WARN("eSE driver version is %06x-%s-%s\n", LINUX_VERSION_CODE,
             DRIVER_VERSION, ESE_VERSION);
...
{
    int ret;
    TMS_INFO("eSE driver init is called!\n");
...
EXPORT_SYMBOL(ese_driver_init);

void ese_driver_exit(void)
{
    TMS_INFO("Remove eSE driver\n");
    spi_unregister_driver(&ese_spi_driver);
}
EXPORT_SYMBOL(ese_driver_exit);
#else
#if defined(CONFIG_TMS_ESE_DEVICE_MODULE)
module_spi_driver(ese_spi_driver);
#else
static int __init ese_driver_init(void)
{
    int ret;
...
static void __exit ese_driver_exit(void)
{
    TMS_INFO("Unloading eSE driver\n");
    spi_unregister_driver(&ese_spi_driver);
}

module_init(ese_driver_init);
module_exit(ese_driver_exit);
#endif
#endif
