...
void sortAssocReqIE(IN P_ADAPTER_T prAdapter,
			IN P_MSDU_INFO_T prMsduInfo, IN UINT_8 fgIsReAssoc)
{
	UINT_32 beginOffset, start, end, frameLen, frameSearch;
	UINT_32 eid, exteid, orderIdx;
	UINT_16 txAssocReqIENums;
	const UINT_8 *primary_IE;
	P_MSDU_INFO_T prMsduInfoInOrder;
	PUINT_8 pucBufferInOrder;
	...
	pucBufferInOrder =
		(PUINT_8) ((unsigned long) prMsduInfoInOrder->prPacket);

	end = prMsduInfo->u2FrameLength;
	if (fgIsReAssoc) {
		start = beginOffset =
			OFFSET_OF(WLAN_ASSOC_REQ_FRAME_T, aucInfoElem[0]);
	...
	while (orderIdx < txAssocReqIENums) {
		...
		primary_IE = kalFindIeExtIE(eid, exteid,
			(PUINT_8)prMsduInfo->prPacket + start,
			(end - start));
		if (primary_IE) {
			frameSearch += IE_SIZE(primary_IE);
			if (frameSearch > frameLen)
				break;
			start = (primary_IE - (PUINT_8)prMsduInfo->prPacket)
				+ IE_SIZE(primary_IE);
	...
	if (frameSearch != frameLen) {
		...
		kalMemCopy((PUINT_8) ((unsigned long)
