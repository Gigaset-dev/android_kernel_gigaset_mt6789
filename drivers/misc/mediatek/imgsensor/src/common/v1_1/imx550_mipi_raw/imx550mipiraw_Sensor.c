...
#include "imx550mipiraw_Sensor.h"
//#include "imx550_eeprom.h"
//drv add by lipengpeng 20230323 start 
#include <linux/of_gpio.h>
#include <linux/gpio.h>
//drv add by lipengpeng 20230323 end
#define PFX "IMX550_camera_sensor"

#define DEVICE_VERSION_IMX550     "imx550"
...
//	.module_id = 0x01,  /* 0x01 Sunny,0x05 QTEK */
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,//SENSOR_OUTPUT_FORMAT_RAW_R,// SENSOR_OUTPUT_FORMAT_RAW_B  SENSOR_OUTPUT_FORMAT_RAW_Gr  SENSOR_OUTPUT_FORMAT_RAW_Gb
	//[agold][xfl][20190313][end]

	.mclk = 24,
	.mipi_lane_num = SENSOR_MIPI_4_LANE, //SENSOR_MIPI_3_LANE
...
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312},/*Preview*/
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /* capture */
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /* video */
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /*hs video*/
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /* slim video*/
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /*custom1  */
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /*custom2 */
{5760, 4312,  0,    0, 5760, 4312, 5760, 4312,    0,    0, 5760, 4312,    0,    0, 5760, 4312}, /* custom3 */
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = {
	...
	{0x05, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 ...
	 0x00, 0x30, 0x0500, 0x05F0, 0x00, 0x00, 0x0000, 0x0000}, /*VC2:Y HIST(3HDR), VC3:AE HIST(3HDR)*/   //59c=1463  0x06b8=1720
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_binning = {
	...
	.i4OffsetY = 0, //PD_OFFSET_Y=16;
	.i4PitchX  = 0, //PD_PITCH_X=8;
	.i4PitchY  = 0, //PD_PITCH_Y=16;
	.i4PairNum  = 0,
	.i4SubBlkW  = 0,  //PD_DENSITY_X=8;
	.i4SubBlkH  = 0,  //PD_DENSITY_Y=2;
	.i4PosL = { {3, 17}, {11, 17}, {1, 19}, {9, 19}, {5, 21}, {13, 21}, {7, 23}, {15, 23},
	            {3, 25}, {11, 25}, {1, 27}, {9, 27}, {5, 29}, {13, 29}, {7, 31}, {15, 31}},
	            
	.i4PosR = { {2, 17}, {10, 17}, {0, 19}, {8, 19}, {4, 21}, {12, 21}, {6, 23}, {14, 23},
	            {2, 25}, {10, 25}, {0, 27}, {8, 27}, {4, 29}, {12, 29}, {6, 31}, {14, 31}},
	.i4BlockNumX = 0,  //PD_BLOCK_NUM_X=574;
	.i4BlockNumY = 0,  //PD_BLOCK_NUM_Y=215;
	.iMirrorFlip = 0,
...
#define IMX550MIPI_MaxGainIndex (223)
kal_uint16 IMX550MIPI_sensorGainMapping[IMX550MIPI_MaxGainIndex][2] ={
	{72,114},
	{76,162},
	{80,205},
	{84,244},
...
static void imx550_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx550_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX550MIPI_MaxGainIndex; i++) {
		if(gain <= IMX550MIPI_sensorGainMapping[i][0]){
			break;
		}
	}
	if(gain != IMX550MIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX550MIPI_sensorGainMapping[i][0]);
	return IMX550MIPI_sensorGainMapping[i][1];
}
...
static kal_uint16 imx550_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
kal_uint16 addr_data_pair_init_imx550[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x3C7E, 0x02,
	0x3C7F, 0x05,
...
kal_uint16 addr_data_pair_preview_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
static kal_uint16 addr_data_pair_capture_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
/*static kal_uint16 addr_data_pair_normal_video_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
*/
static kal_uint16 addr_data_pair_hs_video_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
static kal_uint16 addr_data_pair_slim_video_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
/*static kal_uint16 addr_data_pair_custom1_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
static kal_uint16 addr_data_pair_custom2_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
static kal_uint16 addr_data_pair_custom3_imx550[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x18,
...
{
	LOG_INF("E init\n");
	imx550_table_write_cmos_sensor(addr_data_pair_init_imx550,
		sizeof(addr_data_pair_init_imx550) / sizeof(kal_uint16));
	LOG_INF("L\n");
} /*	sensor_init  */
...
static void preview_setting(void)
{
	...
	imx550_table_write_cmos_sensor(addr_data_pair_preview_imx550,
         sizeof(addr_data_pair_preview_imx550) / sizeof(kal_uint16));  
...
static void capture_setting(kal_uint16 currefps, kal_bool stream_on)
{
	...
	imx550_table_write_cmos_sensor(addr_data_pair_capture_imx550,
		sizeof(addr_data_pair_capture_imx550) / sizeof(kal_uint16));
...
{
	LOG_INF("E\n");
	imx550_table_write_cmos_sensor(addr_data_pair_hs_video_imx550,
		sizeof(addr_data_pair_hs_video_imx550) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E\n");
	imx550_table_write_cmos_sensor(addr_data_pair_slim_video_imx550,
		sizeof(addr_data_pair_slim_video_imx550) / sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	...
	LOG_INF("E\n");
#if 0
	imx550_table_write_cmos_sensor(addr_data_pair_custom1_imx550,
		sizeof(addr_data_pair_custom1_imx550) / sizeof(kal_uint16));
#else
	preview_setting();
#endif
}
...
static void custom2_setting(void)
{
	...
	LOG_INF("E\n");
	imx550_table_write_cmos_sensor(addr_data_pair_custom2_imx550,
		sizeof(addr_data_pair_custom2_imx550) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	LOG_INF("E\n");
	imx550_table_write_cmos_sensor(addr_data_pair_custom3_imx550,
		sizeof(addr_data_pair_custom3_imx550) / sizeof(kal_uint16));
}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
            ...
            printk(
                "imx550 read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",
                read_cmos_sensor_8(0x0016),
                read_cmos_sensor_8(0x0017),
                read_cmos_sensor(0x0000));
				
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("imx550 i2c write id: 0x%x, sensor id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	printk("XiaoPanTimer: 2023-02-27 19.54\n");
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
        spin_unlock(&imgsensor_drv_lock);
...
static void imx550_set_lsc_reg_setting(kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
...
static void set_imx550_ATR(kal_uint16 LimitGain, kal_uint16 LtcRate, kal_uint16 PostGain)
{


}

static kal_uint32 imx550_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_HDR_ATR:
			LOG_INF("SENSOR_FEATURE_SET_HDR_ATR Limit_Gain=%d, LTC Rate=%d, Post_Gain=%d\n",
					...
					(UINT16)*(feature_data + 1),
					(UINT16)*(feature_data + 2));
			set_imx550_ATR((UINT16)*feature_data,
						(UINT16)*(feature_data + 1),
						(UINT16)*(feature_data + 2));
			break;
		...
		case SENSOR_FEATURE_SET_AWB_GAIN:
			imx550_awb_gain(pSetSensorAWB);
			break;
		case SENSOR_FEATURE_SET_LSC_TBL:
			{
				kal_uint8 index = *(((kal_uint8 *)feature_para) + (*feature_para_len));

				imx550_set_lsc_reg_setting(index, feature_data_16, (*feature_para_len)/sizeof(UINT16));
			}
			break;
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			printk("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
				(UINT16) *feature_data);
			PDAFinfo =
		...
		case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
			pr_debug("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			imx550_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
						   , feature_data_16);
			break;			
		case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
			pr_debug("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			imx550_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
						   , feature_data_16);
			break;
...
//drv add by lipengpeng 20230519 start 
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
//drv add by lipengpeng 20230519 end 
			break;
			}
			...
			*feature_para_len = 4;
			break;
		case SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO: //?????€??a?????¡ã¨¨¡¥¡¤??????1??¡ê¨¨?¡§?¡±¡¥????¨C¡ã?¡ë?framesync???????o???a?????¡ã??????margin?€????AE/CCU????¡±¡§
			/*
