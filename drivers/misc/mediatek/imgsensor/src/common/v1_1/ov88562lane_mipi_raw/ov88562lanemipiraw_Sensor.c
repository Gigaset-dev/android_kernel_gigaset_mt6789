...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV88562LANE_SENSOR_ID,		/*record sensor id defined in Kd_imgsensor.h*/
...
//drv add by lipengpeng 20230323 start 
	 //99 118 
	printk("lpp-ov88562lane--gpio_get_value(118)=%d gpio_get_value(408)=%d\n",gpio_get_value(389),gpio_get_value(408));
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("ov88562lane i2c write id: 0x%x, sensor id: 0x%x, ver = 0x%x<0xB1=r2a,0xB0=r1a>\n",
					imgsensor.i2c_write_id, *sensor_id, read_cmos_sensor(0x302A));
				return ERROR_NONE;
			}
			printk("ov88562lane Read sensor id fail,write_id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
