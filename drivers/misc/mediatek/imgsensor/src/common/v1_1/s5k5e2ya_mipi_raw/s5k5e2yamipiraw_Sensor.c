...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,  //SENSOR_OUTPUT_FORMAT_RAW_Gb SENSOR_OUTPUT_FORMAT_RAW_B
	.mclk = 24,
...
static struct imgsensor_struct imgsensor = {
	.mirror = IMAGE_HV_MIRROR,				//mirrorflip information  IMAGE_HV_MIRROR IMAGE_V_MIRROR  IMAGE_H_MIRROR IMAGE_NORMAL
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk(" s5k5e2ya i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("s5k5e2ya Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
