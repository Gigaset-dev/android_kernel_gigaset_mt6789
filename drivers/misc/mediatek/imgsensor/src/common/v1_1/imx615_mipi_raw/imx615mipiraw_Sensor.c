...
static int tekhw_imx615_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_imx615_color =0xff;
static int tekhw_times=0;
#endif
#define PFX "IMX615_camera_sensor"

#define DEVICE_VERSION_IMX615     "imx615"
#define LOG_INF(format, args...)	printk(PFX "[%s] " format, __func__, ##args)
static DEFINE_SPINLOCK(imgsensor_drv_lock);

//extern void IMX615_MIPI_update_awb(kal_uint8 i2c_write_id);
//extern bool imx615CheckVersion(kal_uint32 imgsensor_writeid);
//#define IMX615_LRC

#define DATA_SIZE 1560
#define BYTE      unsigned char
static BYTE imx615_qsc_data[DATA_SIZE] = { 0 };
//extern bool imx615_read_otp_qsc(BYTE* data);

//#define IMX615_LRC 1
#define LRC_SIZE 260

#ifdef IMX615_LRC
static BYTE imx615_lrc_data[LRC_SIZE] = { 0 };
#endif

//extern bool imx615_read_otp_lrc(BYTE* data);
...
{ 6560,  4928,  0,  0,  6560,  4928,  3280,  2464,  0000,  0000,  3280,  2464,  0,  0,  3280,  2464}, /* capture*/
...
{ 6560,  4928,  0,  0,  6560,  4928,  6560,  4928,  0000,  0000,  6560,  4928,  0,  0,  6560,  4928},
//{ 6560,  4928,  0,  0,  6560,  4928,  3280,  2464,  0000,  0000,  3280,  2464,  0,  0,  3280,  2464}, /*custom4 stero@34fps*/
};
...
#define IMX615MIPI_MaxGainIndex (223)
kal_uint16 IMX615MIPI_sensorGainMapping[IMX615MIPI_MaxGainIndex][2] ={
	{72,114},
	{76,162},
	{80,205},
	{84,244},
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX615MIPI_MaxGainIndex; i++) {
		if(gain <= IMX615MIPI_sensorGainMapping[i][0]){
			break;
		}
	}
	if(gain != IMX615MIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX615MIPI_sensorGainMapping[i][0]);
	return IMX615MIPI_sensorGainMapping[i][1];
}
...
#ifdef IMX615_LRC
extern unsigned int read_imx615_LRC(BYTE *data);
static void write_imx615_LRC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < 130; i++) {
		write_cmos_sensor(0x7A98 + i, imx615_lrc_data[i]);
		/* LOG_INF("imx615_lrc_data[%d] = 0x%x\n", i, imx615_lrc_data[i]); */
	}
	
	for (i = 130; i < LRC_SIZE; i++) {
		write_cmos_sensor(0x7B1C + i, imx615_lrc_data[i]);
		/* LOG_INF("imx615_lrc_data[%d] = 0x%x\n", i, imx615_lrc_data[i]); */
	}
}
#endif
static void write_imx615_QSC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < DATA_SIZE; i++) {
		write_cmos_sensor(0xC500 + i, imx615_qsc_data[i]);
		/* LOG_INF("imx615_qsc_data[%d] = 0x%x\n", i, imx615_qsc_data[i]); */
	}
}

kal_uint16 addr_data_pair_init_imx615[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x3C7E, 0x04,
	0x3C7F, 0x02,
...
kal_uint16 addr_data_pair_preview_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
#if 1
kal_uint16 addr_data_pair_custom1_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
kal_uint16 addr_data_pair_custom2_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
kal_uint16 addr_data_pair_custom3_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
kal_uint16 addr_data_pair_hs_video_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
kal_uint16 addr_data_pair_capture_imx615[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x2C,
...
kal_uint16 addr_data_pair_slim_video_imx615[] = {
...
{
	LOG_INF("E init\n");
	imx615_table_write_cmos_sensor(addr_data_pair_init_imx615,
		sizeof(addr_data_pair_init_imx615) / sizeof(kal_uint16));
	LOG_INF("L\n");
} /*	sensor_init  */
...
static void preview_setting(void)
{
	...
	imx615_table_write_cmos_sensor(addr_data_pair_preview_imx615,
         sizeof(addr_data_pair_preview_imx615) / sizeof(kal_uint16));  
...
{
	LOG_INF("E\n");
	imx615_table_write_cmos_sensor(addr_data_pair_hs_video_imx615,
		sizeof(addr_data_pair_hs_video_imx615) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E\n");
	imx615_table_write_cmos_sensor(addr_data_pair_slim_video_imx615,
         sizeof(addr_data_pair_slim_video_imx615) / sizeof(kal_uint16));  
}
...
static void custom1_setting(void)
{
	...
	LOG_INF("E\n");
#if 0
	imx615_table_write_cmos_sensor(addr_data_pair_custom1_imx615,
		sizeof(addr_data_pair_custom1_imx615) / sizeof(kal_uint16));
#else
	imx615_table_write_cmos_sensor(addr_data_pair_custom1_imx615,
		sizeof(addr_data_pair_custom1_imx615) / sizeof(kal_uint16));
#endif
...
static void custom2_setting(void)
{
	...
	LOG_INF("E\n");
	imx615_table_write_cmos_sensor(addr_data_pair_custom2_imx615,
		sizeof(addr_data_pair_custom2_imx615) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	LOG_INF("E\n");
	imx615_table_write_cmos_sensor(addr_data_pair_capture_imx615,
		sizeof(addr_data_pair_capture_imx615) / sizeof(kal_uint16));
}
...
static kal_uint32 open(void)
{
	...
	sensor_init();
#ifdef IMX615_LRC	
	read_imx615_LRC(imx615_lrc_data);
	//LOG_INF("[%s][%d]imx615_lrc_data[0]:0x%x   imx615_lrc_data[383]:0x%x\n", __func__, __LINE__,imx615_lrc_data[0],imx615_lrc_data[260]);
	write_imx615_LRC_Data();
#endif	
	//IMX615_MIPI_update_awb(imgsensor.i2c_write_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_imx615_mirror)
    {
	    set_mirror_flip(tekhw_imx615_mirror);
    }
#else
...
{
    tekhw_camera_data* tekhw_imx615_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_IMX615_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_imx615_ptr != NULL) && (tekhw_imx615_ptr->set_mirror.size > 0))
    {
        tekhw_imx615_mirror = *(tekhw_u16*)tekhw_imx615_ptr->set_mirror.buffer;
    }
    if ((tekhw_imx615_ptr != NULL) && (tekhw_imx615_ptr->set_color.size > 0))
    {
		tekhw_imx615_color= *(tekhw_u16*)tekhw_imx615_ptr->set_color.buffer;
    }
}
#endif
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(tekhw_imx615_color!=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_imx615_color;
	}
	tekhw_times =1;
#endif
...
static void imx615_set_lsc_reg_setting(kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
...
static void set_imx615_ATR(kal_uint16 LimitGain, kal_uint16 LtcRate, kal_uint16 PostGain)
{


}

static kal_uint32 imx615_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		case SENSOR_FEATURE_SET_HDR_ATR:
			...
			set_imx615_ATR((UINT16)*feature_data,
						(UINT16)*(feature_data + 1),
						(UINT16)*(feature_data + 2));
			break;
		...
		case SENSOR_FEATURE_SET_AWB_GAIN:
			imx615_awb_gain(pSetSensorAWB);
			break;
		case SENSOR_FEATURE_SET_LSC_TBL:
			{
				kal_uint8 index = *(((kal_uint8 *)feature_para) + (*feature_para_len));

				imx615_set_lsc_reg_setting(index, feature_data_16, (*feature_para_len)/sizeof(UINT16));
			}
			break;
