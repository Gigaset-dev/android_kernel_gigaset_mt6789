...
BYTE imx350ys2_DCC_data[384] = { 0 };	/* 16x12x2 */
...
void imx350ys2_read_SPC(BYTE *data)
{
...
void imx350ys2_read_DCC(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	/* int i; */
	addr = DCC_START_ADDR;
	size = 384;
	if (!get_done_dcc || last_size_dcc != size) {
		if (!imx350_read_eeprom(addr, imx350ys2_DCC_data, size)) {
			get_done_dcc = 0;
			last_size_dcc = 0;
			last_offset_dcc = 0;
			/* return false; */
	...
	memcpy(data, imx350ys2_DCC_data, size);
	/* return true; */
}
