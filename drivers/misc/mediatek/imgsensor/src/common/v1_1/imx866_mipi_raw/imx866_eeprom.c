...
#define IMX866_I2C_SPEED       100
#define IMX866_MAX_OFFSET      0xFFFF
...
static DEFINE_MUTEX(gimx866_eeprom_mutex);

static bool selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
	char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	if (addr > IMX866_MAX_OFFSET)
		return false;

	if (iReadRegI2C(pu_send_cmd, 2, (u8 *) data,
			1, IMX866_EEPROM_READ_ID) < 0) {
		return false;
	}
	return true;
}

static bool read_imx866_eeprom(kal_uint16 addr, BYTE *data, int size)
{
	int i = 0;
	int offset = addr;
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(void)
{
	...
	mutex_lock(&gimx866_eeprom_mutex);
	if (pinfo == NULL) {
		read_imx866_eeprom(0x1, read_data, 4);
		if (((read_data[3] << 24) |
		     (read_data[2] << 16) |
		     (read_data[1] << 8) |
	...
	mutex_unlock(&gimx866_eeprom_mutex);
...
unsigned int read_imx866_LRC(BYTE *data)
{
	...
	LOG_INF("read imx866 LRC, addr = %d, size = %u\n",
		pinfo->LRC_addr, pinfo->LRC_size);

	mutex_lock(&gimx866_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx866_eeprom(pinfo->LRC_addr,
			       IMX866_LRC_data, pinfo->LRC_size)) {
		readed_size = pinfo->LRC_size;
	}
	mutex_unlock(&gimx866_eeprom_mutex);

	memcpy(data, IMX866_LRC_data, pinfo->LRC_size);
	return readed_size;
}
...
{
	static BYTE IMX866_DCC_data[DCC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info();

	LOG_INF("read imx866 DCC, addr = %d, size = %u\n",
		pinfo->DCC_addr, pinfo->DCC_size);

	mutex_lock(&gimx866_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx866_eeprom(pinfo->DCC_addr,
			       IMX866_DCC_data, pinfo->DCC_size)) {
		readed_size = pinfo->DCC_size;
	}
	mutex_unlock(&gimx866_eeprom_mutex);

	memcpy(data, IMX866_DCC_data, pinfo->DCC_size);
	return readed_size;
}
