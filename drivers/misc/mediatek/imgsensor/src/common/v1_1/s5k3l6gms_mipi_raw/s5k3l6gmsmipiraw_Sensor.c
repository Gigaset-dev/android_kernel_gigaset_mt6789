...
#include "s5k3l6gmsmipiraw_Sensor.h"
...
#define LOG_1 LOG_INF("S5K3L6GMS,MIPI 4LANE\n")
#define S5K3L6GMS_MIRROR_FLIP 0
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6GMS_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
static void check_streamon(void)
{
	...
	mdelay(3);
	for (i = 0; i < timeout; i++) {
		ret = read_cmos_sensor_byte(0x0005);
		if ( ret == 0xFF)
			mdelay(5);
		else
			break;
	}
	LOG_INF("%s exit at %d [05]:%x!\n", __func__,i,ret);
}
...
UINT32 S5K3L6GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("S5K3L6GMS_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L6GMS_MIPI_RAW_SensorInit	*/
