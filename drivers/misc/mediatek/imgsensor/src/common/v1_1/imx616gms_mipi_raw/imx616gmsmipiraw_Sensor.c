...
#include "imx616gmsmipiraw_Sensor.h"
//#include "imx616gms_eeprom.h"
//#define XUNHU_LPS_TEKHW_SUPPORT
#ifdef XUNHU_LPS_TEKHW_SUPPORT
#include <teksunhw.h>/*add by xunhu andy andy20160720 at 22:49*/
static int tekhw_imx616gms_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_imx616gms_color =0xff;
static int tekhw_times=0;
#endif
#define PFX "IMX616GMS_camera_sensor"

#define DEVICE_VERSION_IMX616GMS     "imx616gms"
#define LOG_INF(format, args...)	printk(PFX "[%s] " format, __func__, ##args)
static DEFINE_SPINLOCK(imgsensor_drv_lock);

//extern void IMX616GMS_MIPI_update_awb(kal_uint8 i2c_write_id);
//extern bool imx616gmsCheckVersion(kal_uint32 imgsensor_writeid);
//#define IMX616GMS_LRC

#define DATA_SIZE 1560
#define BYTE      unsigned char
static BYTE imx616gms_qsc_data[DATA_SIZE] = { 0 };
//extern bool imx616gms_read_otp_qsc(BYTE* data);

//#define IMX616GMS_LRC 1
#define LRC_SIZE 260

#ifdef IMX616GMS_LRC
static BYTE imx616gms_lrc_data[LRC_SIZE] = { 0 };
#endif

//extern bool imx616gms_read_otp_lrc(BYTE* data);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX616GMS_SENSOR_ID,
...
#define IMX616GMSMIPI_MaxGainIndex (223)
kal_uint16 IMX616GMSMIPI_sensorGainMapping[IMX616GMSMIPI_MaxGainIndex][2] ={
	{72,114},
	{76,162},
	{80,205},
	{84,244},
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX616GMSMIPI_MaxGainIndex; i++) {
		if(gain <= IMX616GMSMIPI_sensorGainMapping[i][0]){
			break;
		}
	}
	if(gain != IMX616GMSMIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX616GMSMIPI_sensorGainMapping[i][0]);
	return IMX616GMSMIPI_sensorGainMapping[i][1];
}
...
static kal_uint16 imx616gms_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
#ifdef IMX616GMS_LRC
extern unsigned int read_imx616gms_LRC(BYTE *data);
static void write_imx616gms_LRC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < 130; i++) {
		write_cmos_sensor(0x7A98 + i, imx616gms_lrc_data[i]);
		/* LOG_INF("imx616gms_lrc_data[%d] = 0x%x\n", i, imx616gms_lrc_data[i]); */
	}
	
	for (i = 130; i < LRC_SIZE; i++) {
		write_cmos_sensor(0x7B1C + i, imx616gms_lrc_data[i]);
		/* LOG_INF("imx616gms_lrc_data[%d] = 0x%x\n", i, imx616gms_lrc_data[i]); */
	}
}
#endif
static void write_imx616gms_QSC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < DATA_SIZE; i++) {
		write_cmos_sensor(0xC500 + i, imx616gms_qsc_data[i]);
		/* LOG_INF("imx616gms_qsc_data[%d] = 0x%x\n", i, imx616gms_qsc_data[i]); */
	}
}

kal_uint16 addr_data_pair_init_imx616gms[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x3C7E, 0x01,
	0x3C7F, 0x15,
...
kal_uint16 addr_data_pair_preview_imx616gms[] = {
	0x0112,0x0A,
	0x0113,0x0A,
	0x0114,0x03,
	0x0342,0x1A,
...
#if 1
kal_uint16 addr_data_pair_custom1_imx616gms[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x3C,
...
kal_uint16 addr_data_pair_custom2_imx616gms[] = {
...
kal_uint16 addr_data_pair_custom3_imx616gms[] = {
...
kal_uint16 addr_data_pair_hs_video_imx616gms[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x15,
	...
	#if 0
kal_uint16 addr_data_pair_capture_imx616gms[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x1C,
...
#endif
kal_uint16 addr_data_pair_slim_video_imx616gms[] = {
...
{
	LOG_INF("E init\n");
	imx616gms_table_write_cmos_sensor(addr_data_pair_init_imx616gms,
		sizeof(addr_data_pair_init_imx616gms) / sizeof(kal_uint16));
	LOG_INF("L\n");
} /*	sensor_init  */
...
static void preview_setting(void)
{
	...
	imx616gms_table_write_cmos_sensor(addr_data_pair_preview_imx616gms,
         sizeof(addr_data_pair_preview_imx616gms) / sizeof(kal_uint16));  
...
static void capture_setting(kal_uint16 currefps, kal_bool stream_on)
{
	...
	//imx616gms_table_write_cmos_sensor(addr_data_pair_capture_imx616gms,
    //     sizeof(addr_data_pair_capture_imx616gms) / sizeof(kal_uint16));  
	preview_setting();
	LOG_INF("L!\n");
}
...
{
	LOG_INF("E\n");
	imx616gms_table_write_cmos_sensor(addr_data_pair_hs_video_imx616gms,
		sizeof(addr_data_pair_hs_video_imx616gms) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E\n");
	imx616gms_table_write_cmos_sensor(addr_data_pair_slim_video_imx616gms,
         sizeof(addr_data_pair_slim_video_imx616gms) / sizeof(kal_uint16));  
}
...
static void custom1_setting(void)
{
	...
	LOG_INF("E\n");
#if 0
	imx616gms_table_write_cmos_sensor(addr_data_pair_custom1_imx616gms,
		sizeof(addr_data_pair_custom1_imx616gms) / sizeof(kal_uint16));
#else
	imx616gms_table_write_cmos_sensor(addr_data_pair_custom1_imx616gms,
		sizeof(addr_data_pair_custom1_imx616gms) / sizeof(kal_uint16));
#endif
...
static void custom2_setting(void)
{
	...
	LOG_INF("E\n");
	imx616gms_table_write_cmos_sensor(addr_data_pair_custom2_imx616gms,
		sizeof(addr_data_pair_custom2_imx616gms) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	LOG_INF("E\n");
	imx616gms_table_write_cmos_sensor(addr_data_pair_custom3_imx616gms,
		sizeof(addr_data_pair_custom3_imx616gms) / sizeof(kal_uint16));
}
...
//	AFRegulatorCtrl(1);
	...
	sensor_init();
#ifdef IMX616GMS_LRC	
	read_imx616gms_LRC(imx616gms_lrc_data);
	//LOG_INF("[%s][%d]imx616gms_lrc_data[0]:0x%x   imx616gms_lrc_data[383]:0x%x\n", __func__, __LINE__,imx616gms_lrc_data[0],imx616gms_lrc_data[260]);
	write_imx616gms_LRC_Data();
#endif	
	//IMX616GMS_MIPI_update_awb(imgsensor.i2c_write_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_imx616gms_mirror)
    {
	    set_mirror_flip(tekhw_imx616gms_mirror);
    }
#else
...
{
    tekhw_camera_data* tekhw_imx616gms_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_IMX616GMS_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_imx616gms_ptr != NULL) && (tekhw_imx616gms_ptr->set_mirror.size > 0))
    {
        tekhw_imx616gms_mirror = *(tekhw_u16*)tekhw_imx616gms_ptr->set_mirror.buffer;
    }
    if ((tekhw_imx616gms_ptr != NULL) && (tekhw_imx616gms_ptr->set_color.size > 0))
    {
		tekhw_imx616gms_color= *(tekhw_u16*)tekhw_imx616gms_ptr->set_color.buffer;
    }
}
#endif
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(tekhw_imx616gms_color!=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_imx616gms_color;
	}
	tekhw_times =1;
#endif
...
static void imx616gms_set_lsc_reg_setting(kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
...
static void set_imx616gms_ATR(kal_uint16 LimitGain, kal_uint16 LtcRate, kal_uint16 PostGain)
{


}

static kal_uint32 imx616gms_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		case SENSOR_FEATURE_SET_HDR_ATR:
			...
			set_imx616gms_ATR((UINT16)*feature_data,
						(UINT16)*(feature_data + 1),
						(UINT16)*(feature_data + 2));
			break;
...
//			imgsensor.hdr_mode = (UINT8)*feature_data_32;
		...
		case SENSOR_FEATURE_SET_AWB_GAIN:
			imx616gms_awb_gain(pSetSensorAWB);
			break;
		case SENSOR_FEATURE_SET_LSC_TBL:
			{
				kal_uint8 index = *(((kal_uint8 *)feature_para) + (*feature_para_len));

				imx616gms_set_lsc_reg_setting(index, feature_data_16, (*feature_para_len)/sizeof(UINT16));
			}
			break;
...
UINT32 IMX616GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
