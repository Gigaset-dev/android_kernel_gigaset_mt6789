 ...
 *============================================================================
 ****************************************************************************/
#define PFX "IMX481LH_camera_sensor"
...
#include "imx481lhmipiraw_Sensor.h"
...
#endif
static kal_uint16 imx481lh_table_write_cmos_sensor(
...
kal_uint16 addr_data_pair_init_imx481lh[] = {
...
{
	pr_debug("02E\n");
	imx481lh_table_write_cmos_sensor(addr_data_pair_init_imx481lh,
	    sizeof(addr_data_pair_init_imx481lh)/sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_preview_imx481lh[] = {
	/* MIPI output setting */
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
...
{
	imx481lh_table_write_cmos_sensor(
	addr_data_pair_preview_imx481lh,
	sizeof(addr_data_pair_preview_imx481lh) / sizeof(kal_uint16));
} /* preview_setting */


kal_uint16 addr_data_pair_capture_imx481lh[] = {
	/* MIPI output setting */
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
...
{
	imx481lh_table_write_cmos_sensor(addr_data_pair_capture_imx481lh,
		sizeof(addr_data_pair_capture_imx481lh) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_video_imx481lh[] = {
	/* MIPI output setting */
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
...
{
	pr_debug("E! %s:%d\n", __func__, currefps);
	imx481lh_table_write_cmos_sensor(addr_data_pair_video_imx481lh,
		sizeof(addr_data_pair_video_imx481lh) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_hs_video_imx481lh[] = {	/*720 120fps */
	/* MIPI output setting */
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
...
{
	imx481lh_table_write_cmos_sensor(addr_data_pair_hs_video_imx481lh,
		sizeof(addr_data_pair_hs_video_imx481lh) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_slim_video_imx481lh[] = {
	/* MIPI output setting */
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
...
{
	imx481lh_table_write_cmos_sensor(addr_data_pair_slim_video_imx481lh,
	    sizeof(addr_data_pair_slim_video_imx481lh) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	kal_uint16 imx481_otp_flag = 0;
	kal_uint16 imx481_otp_state = 0;
		...
		do {
			*sensor_id = return_lot_id_from_otp()+2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				write_cmos_sensor(0x0136, 0x0C);
				write_cmos_sensor(0x0137, 0x00);
				write_cmos_sensor(0x0A02, 0x00); //Select Page
				write_cmos_sensor(0x0A00, 0x01); //Read ON
				 mdelay(20);
				 imx481_otp_state = read_cmos_sensor(0x0A01);
				 if((imx481_otp_state == 0) || (imx481_otp_state == 1)){
				 imx481_otp_flag = read_cmos_sensor(0x0A04);   
				 }
				printk("liaojie imx481_otp_flag=0x%x\n",imx481_otp_flag);
				if(imx481_otp_flag == 0x1b)
				{
					printk("liaojie check fail\n");
...
static kal_uint32 imx481lh_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
...
static void check_stream_is_on(void)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		if (framecnt != 0xFF) {
			pr_debug("IMX481LH stream is on, %d \\n", framecnt);
			break;
		}
		pr_debug("IMX481LH stream is not on %d \\n", framecnt);
		mdelay(1);
	}
}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		imx481lh_awb_gain(pSetSensorAWB);
		break;
...
UINT32 IMX481LH_MIPI_RAW_SensorInit(
