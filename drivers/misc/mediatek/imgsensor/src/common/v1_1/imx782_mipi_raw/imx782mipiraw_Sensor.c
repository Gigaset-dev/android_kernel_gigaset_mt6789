...
#include "imx782mipiraw_Sensor.h"
//#include "imx782_eeprom.h"

#define PFX "imx782_camera_sensor"

#define DEVICE_VERSION_imx782     "imx782"
...
#define LRC_SQC_LOG(format, args...)
#endif
//extern void imx782_MIPI_update_awb(kal_uint8 i2c_write_id);
//extern bool Imx586CheckVersion(kal_uint32 imgsensor_writeid);

// prize add by chenwenhui for remosaic start
#define REMOSAIC_Enable 0
// prize add by chenwenhui for remosaic end

#define Long_Exp_16s 0
#define PDAF_MODE_SUPPORT  0
#define DATA_SIZE 3024 //2304
#define BYTE      unsigned char
static BYTE imx782_qsc_data[DATA_SIZE] = { 0 };
//extern bool imx782_read_otp_qsc(BYTE* data);

#define LRC_SIZE 504 //384
static BYTE imx782_lrc_data[LRC_SIZE] = { 0 };
//extern bool imx782_read_otp_lrc(BYTE* data);
static unsigned char LRC_SQC_readed = 0;
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX782_SENSOR_ID,
	...
	.mipi_sensor_type = MIPI_OPHY_NCSI2, //MIPI_OPHY_NCSI2, /* 0,MIPI_OPHY_NCSI2;  1,MIPI_OPHY_CSI2 */
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] = {
	...
	{9248, 6944, 784, 1312, 7680, 4320, 1920, 1080, 0, 0, 1920, 1080,  0,  0, 1920, 1080}, // hs_video  1920x1080 @120fps
	{9248, 6944, 784, 1312, 7680, 4320, 1280,  720, 0, 0, 1280,  720,  0,  0, 1280,  720},
	{9248, 6944,   0,    0, 9248, 6944, 2312, 1736, 0, 0, 2312, 1736,  0,  0, 2312, 1736}, // custom1  2312x1736@120FPS
	{9248, 6944, 784, 1312, 7680, 4320, 3840, 2160, 0, 0, 3840, 2160,  0,  0, 3840, 2160}, // custom2 3840x2160// @60fps
	{9248, 6944,   0,    0, 9248, 6944, 9248, 6944, 0, 0, 9248, 6944,  0,  0, 9248, 6944}, // custom3 9248x6944 @15fps
};
#else
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] = {
	{4624, 3472,   0,   0, 4624, 3472, 4624, 3472, 0, 0, 4624, 3472,  0,  0, 4624, 3472}, /* Preview */
	{4624, 3472,   0,   0, 4624, 3472, 4624, 3472, 0, 0, 4624, 3472,  0,  0, 4624, 3472}, /* capture */
	{4624, 3472,   0,   0, 4624, 3472, 4624, 3472, 0, 0, 4624, 3472,  0,  0, 4624, 3472}, /* normal video */
	{4624, 3472, 392, 656, 3840, 2160, 1920, 1080, 0, 0, 1920, 1080,  0,  0, 1920, 1080}, // hs_video  1920x1080 @120fps
	{4624, 3472,1032,1016, 2560, 1440, 1280,  720, 0, 0, 1280,  720,  0,  0, 1280,  720},
	{4624, 3472,   0,   0, 4624, 3472, 2312, 1736, 0, 0, 2312, 1736,  0,  0, 2312, 1736}, // custom1  2312x1736@120FPS
	{4624, 3472, 392, 656, 3840, 2160, 3840, 2160, 0, 0, 3840, 2160,  0,  0, 3840, 2160}, // custom2 3840x2160// @60fps
	{4624, 3472,   0,   0, 4624, 3472, 4624, 3472, 0, 0, 4624, 3472,  0,  0, 4624, 3472},
};
#endif
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_binning = {
	 .i4OffsetX = 17,//PD_OFFSET_X=17;
	 .i4OffsetY = 16,//PD_OFFSET_Y=16;
	 .i4PitchX = 8,//PD_PITCH_X=8;
	 .i4PitchY = 16,//PD_PITCH_Y=16;
	 .i4PairNum = 8,
	 .i4SubBlkW = 8,//PD_DENSITY_X=8;
	 .i4SubBlkH = 2,//PD_DENSITY_Y=2;
	 .i4PosL = {{0x20 ,0x17},{0x18 ,0x19},{0x22 ,0x21},{0x24 ,0x23},{0x20 ,0x25},{0x18 ,0x27},{0x22 ,0x29},{0x24 ,0x31}},
	 .i4PosR = {{0x19 ,0x17},{0x17 ,0x19},{0x21 ,0x21},{0x23 ,0x23},{0x19 ,0x25},{0x17 ,0x27},{0x21 ,0x29},{0x23 ,0x31}},
	 .iMirrorFlip = 0,
	 .i4BlockNumX = 574,//PD_BLOCK_NUM_X=574;
	 .i4BlockNumY = 215,//PD_BLOCK_NUM_Y=215;
...
static void imx782_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx782_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
{
	kal_uint16 reg_gain, max_gain = 64 * BASEGAIN;//64X
...
static kal_uint16 imx782_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static void write_imx782_LRC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < 252; i++) {
		write_cmos_sensor_8(0x7B00 + i, imx782_lrc_data[i]);
		LRC_SQC_LOG("imx782_lrc_data[%d] = 0x%x\n", i, imx782_lrc_data[i]);
	}
	
	for (i = 252; i < LRC_SIZE; i++) {
		write_cmos_sensor_8(0x7C00 + i, imx782_lrc_data[i]);
		LRC_SQC_LOG("imx782_lrc_data[%d] = 0x%x\n", i, imx782_lrc_data[i]);
	}
}

static void write_imx782_QSC_Data(void)
{
	kal_uint16 i;	

	for (i = 0; i < DATA_SIZE; i++) {
		write_cmos_sensor_8(0xCA00 + i, imx782_qsc_data[i]);
		LRC_SQC_LOG("imx782_qsc_data[%d] = 0x%x\n", i, imx782_qsc_data[i]);
	}
}

static kal_uint16 imx782_init_setting[] = {
0x0136, 0x18,
0x0137, 0x00,
0x3204, 0x00,
0x33F0, 0x01,
...
static kal_uint16 imx782_capture_setting[] = {
0x0114, 0x03,
0x0342, 0x1E,
0x0343, 0x78,
0x0340, 0x0E,
...
static kal_uint16 imx782_preview_setting[] = {
0x0114, 0x03,
0x0342, 0x1E,
0x0343, 0x78,
0x0340, 0x0E,
...
static kal_uint16 imx782_normal_video_setting[] = {
0x0114, 0x03,
0x0342, 0x1E,
0x0343, 0x78,
0x0340, 0x0E,
...
static kal_uint16 imx782_hs_video_setting[] = {
//1920 x 1080 @120 fps
0x0114, 0x03,
0x0342, 0x0A,
0x0343, 0x38,
...
static kal_uint16 imx782_slim_video_setting[] = {
...
static kal_uint16 imx782_custom1_setting[] = {
//2312x1736@120FPS
0x0112, 0x0A,
0x0113, 0x0A,
0x0114, 0x03,
...
static kal_uint16 imx782_custom2_setting[] = {
// 3840x2160 @60fps   
0x0112, 0x0A,
0x0113, 0x0A,
0x0114, 0x03,
...
static kal_uint16 imx782_custom3_setting[] = {
//reg_B 9248x6944 
...
{
	LOG_INF("E init\n");
	imx782_table_write_cmos_sensor(imx782_init_setting,
		sizeof(imx782_init_setting) / sizeof(kal_uint16));
	LOG_INF("L\n");
} /*	sensor_init  */
...
static void preview_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_preview_setting,
         sizeof(imx782_preview_setting) / sizeof(kal_uint16));  
...
static void capture_setting(kal_uint16 currefps, kal_bool stream_on)
{
	...
	imx782_table_write_cmos_sensor(imx782_capture_setting,
		sizeof(imx782_capture_setting) / sizeof(kal_uint16));
...
static void normal_video_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_normal_video_setting,
         sizeof(imx782_normal_video_setting) / sizeof(kal_uint16));  
}
...
static void hs_video_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_hs_video_setting,
         sizeof(imx782_hs_video_setting) / sizeof(kal_uint16));  
}
...
static void slim_video_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_slim_video_setting,
         sizeof(imx782_slim_video_setting) / sizeof(kal_uint16));  
}
...
write_cmos_sensor(0x0100,0x00); //standby
	...
	imx782_table_write_cmos_sensor(imx782_custom1_setting,
         sizeof(imx782_custom1_setting) / sizeof(kal_uint16));  
}
...
static void custom2_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_custom2_setting,
         sizeof(imx782_custom2_setting) / sizeof(kal_uint16));  
}
...
static void custom3_setting(void)
{
	...
	imx782_table_write_cmos_sensor(imx782_custom3_setting,
         sizeof(imx782_custom3_setting) / sizeof(kal_uint16));  
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
     ...
     */
	printk("start get imx782 sensorid\n");
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
            ...
            LOG_INF(
				...
				printk("imgsensor.i2c_write_id=0x%x,*sensor id=0x%x\n",imgsensor.i2c_write_id,*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(LRC_SQC_readed != 1) {
					...
					if((LRC_SIZE == read_imx782_LRC(imx782_lrc_data)) && (DATA_SIZE == read_imx782_DCC(imx782_qsc_data)))
						LRC_SQC_readed = 1;
...
static kal_uint32 open(void)
{
	...
	lrc_reg = read_cmos_sensor_8(0x340D);
	LOG_INF("lhh LRC_SQC_readed:%d lrc_reg:%d \n",LRC_SQC_readed,lrc_reg);
	if(LRC_SQC_readed){
		LOG_INF("lhh write write_imx782_LRC_Data start \n");
		write_imx782_LRC_Data();
		LOG_INF("lhh write write_imx782_LRC_Data end \n");
	}
	
	//imx782_MIPI_update_awb(imgsensor.i2c_write_id);
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	if(LRC_SQC_readed){
		LOG_INF("lhh write write_imx782_QSC_Data start \n");
		write_imx782_QSC_Data();
		LOG_INF("lhh write write_imx782_QSC_Data end \n");
	}
...
static void imx782_set_lsc_reg_setting(kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
...
static void set_imx782_ATR(kal_uint16 LimitGain, kal_uint16 LtcRate, kal_uint16 PostGain)
{


}

static kal_uint32 imx782_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_HDR_ATR:
			LOG_INF("SENSOR_FEATURE_SET_HDR_ATR Limit_Gain=%d, LTC Rate=%d, Post_Gain=%d\n",
					...
					(UINT16)*(feature_data + 1),
					(UINT16)*(feature_data + 2));
			set_imx782_ATR((UINT16)*feature_data,
						(UINT16)*(feature_data + 1),
						(UINT16)*(feature_data + 2));
			break;
		...
		case SENSOR_FEATURE_SET_AWB_GAIN:
			imx782_awb_gain(pSetSensorAWB);
			break;
		case SENSOR_FEATURE_SET_LSC_TBL:
			{
				kal_uint8 index = *(((kal_uint8 *)feature_para) + (*feature_para_len));

				imx782_set_lsc_reg_setting(index, feature_data_16, (*feature_para_len)/sizeof(UINT16));
			}
			break;
		...
		case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
			pr_debug("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			imx782_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
						   , feature_data_16);
			break;			
		case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
			pr_debug("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
				(*feature_para_len));
			imx782_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
						   , feature_data_16);
			break;
...
UINT32 IMX782_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
