...
static void kbase_sync_fence_debug_dump(struct kbase_context *kctx, struct dma_fence *fence, bool show_atoms)
{
	unsigned long flags;
	...
	switch (katom->core_req & BASE_JD_REQ_SOFT_JOB_TYPE) {
		case BASE_JD_REQ_SOFT_FENCE_TRIGGER:
			...
			if (res == 0) {
				...
				kbase_sync_fence_debug_dump(kctx, info.fence, false);
			}
			break;
		case BASE_JD_REQ_SOFT_FENCE_WAIT:
...
static void kbase_fence_debug_wait_timeout(struct kbase_jd_atom *katom)
{
	...
	kbase_sync_fence_debug_dump(kctx, fence, true);
