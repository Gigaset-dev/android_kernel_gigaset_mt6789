...
//extern bool himax_is_gesture_wakeup_enabled(void);
...
//	if (himax_is_gesture_wakeup_enabled() == 1)
//		gpiod_set_value(ctx->reset_gpio, 1);
//	else
		gpiod_set_value(ctx->reset_gpio, 0);
	devm_gpiod_put(ctx->dev, ctx->reset_gpio);
//	if (himax_is_gesture_wakeup_enabled() == 1) {
//		pr_info("%s himax gesture open\n", __func__);
//	} else {
...
#define DATA_RATE_90				 856

#define PHYSICAL_WIDTH              48989
#define PHYSICAL_HEIGHT             108865

#define PCLK_IN_KHZ_60HZ \
    ((HAC+HFP+HSA+HBP)*(VAC+VFP+VSA+VBP)*(60)/1000)  //60
#define PCLK_IN_KHZ_90HZ \
    ((HAC+HFP+HSA+HBP)*(VAC+VFP_90+VSA+VBP)*(90)/1000)  //90
					...
					struct drm_connector *connector)
{
	...
	connector->display_info.height_mm = 109;
	return 1;
}
