...
int g_doze_en = 0;
EXPORT_SYMBOL(g_doze_en);

int g_hbm_stat = 0;
EXPORT_SYMBOL(g_hbm_stat);
...
{
	// pr_err("%s g_doze_en=%d", __func__, g_doze_en);
	return g_doze_en;
}
EXPORT_SYMBOL_GPL(drv_doze_state);

bool drv_get_hbmstate(void)
{
	// pr_err("%s g_hbm_stat=%d", __func__, g_hbm_stat);
	return g_hbm_stat;
}
EXPORT_SYMBOL_GPL(drv_get_hbmstate);
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    strcpy(current_lcm_info.chip,"rm692e5,cmd,gy");
...
static const struct of_device_id lcm_of_match[] = {
	{
	    .compatible = "visionox,rm692e5,cmd,gy",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-visionox-rm692e5-dphy-cmd-120hz-gy",
		.owner = THIS_MODULE,
