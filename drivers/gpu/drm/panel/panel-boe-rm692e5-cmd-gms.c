...
extern int panel_is_doze_enable;
	...
	void *handle, unsigned int level)
{
	 ...
	 if(level > 255)
	 {
	 	...
	 	{
	 		/*PRIZE:Added by lvyuanchuan,X9-534,20230103*/
	 		level_normal = Gamma_to_level[bl_level] + BLK_LEVEL_OFFSET;//bl_level * BLK_LEVEL_MAP3/255 + BLK_LEVEL_OFFSET;
			bl_tb0[1] = (level_normal>>8)&0xf;
			bl_tb0[2] = (level_normal)&0xff;
			if (!cb)
				return -1;
			pr_err("panel out HBM bl_level = %d\n",bl_level);
...
unsigned short led_level_disp_get_gms(char *name)
{
    int trans_level = 0;
...
EXPORT_SYMBOL(led_level_disp_get_gms);
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    strcpy(current_lcm_info.chip,"rm692e5,cmd,gms");
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "Raydium,rm692e5,cmd,gms", },
	{ }
};
