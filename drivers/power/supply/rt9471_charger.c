...
#define RT9471_ICHG_MINUA		100000
#define RT9471_ICHG_MAXUA		5000000
#define RT9471_CV_MAXUV			4710000
#define RT9471_MANUFACTURER		"Richtek Technology Corp"
#define RT9471_MASK_EN_PUMPX		BIT(7)

static const char *const rt9471_attach_trig_names[] = {
...
/* Power supply */
static char *rt9471_psy_supplied_to[] = {
	"battery",
	"mtk-master-charger"
};

static const enum power_supply_usb_type rt9471_chg_usb_types[] = {
...
static const enum power_supply_property rt9471_chg_properties[] = {
...
static int rt9471_chg_prop_is_writeable(struct power_supply *psy,
...
//#ifdef CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT
	...
	bool bc12_done;
	bool bc12_hz_state;
	enum rt9471_attach_trigger attach_trig;
...
static int __rt9471_get_otgcc(struct rt9471_chip *chip, u32 *ma)
{
	int ret = 0;
	u8 regval = 0;

	ret = rt9471_i2c_read_byte(chip, RT9471_REG_OTGCFG, &regval);
	if (ret < 0)
		return ret;

	regval = (regval & RT9471_OTGCC_MASK) >> RT9471_OTGCC_SHIFT;
	*ma = regval ? rt9471_otgcc[1] : rt9471_otgcc[0];

	return ret;
}

static __maybe_unused int __rt9471_get_otgcv(struct rt9471_chip *chip, u32 *mv)
{
	int ret = 0;
	u8 regval = 0;
	...
	regval = (regval & RT9471_OTGCV_MASK) >> RT9471_OTGCV_SHIFT;
	*mv = rt9471_closest_value(RT9471_OTGCV_MIN, RT9471_OTGCV_MAX,
				     RT9471_OTGCV_STEP, regval);
...
static int __rt9471_is_enabled_otg(struct rt9471_chip *chip, bool *en)
{
	return rt9471_i2c_test_bit(chip, RT9471_REG_FUNCTION,
...
static int __rt9471_set_otgcv(struct rt9471_chip *chip, u32 cv)
{
	u8 regval = 0;

	dev_info(chip->dev, "%s cv = %d\n", __func__, cv);
	regval = rt9471_closest_reg(RT9471_OTGCV_MIN, RT9471_OTGCV_MAX,
				    RT9471_OTGCV_STEP, cv);

	return rt9471_i2c_update_bits(chip, RT9471_REG_OTGCFG,
				      regval << RT9471_OTGCV_SHIFT,
				      RT9471_OTGCV_MASK);
}
...
static int rt9471_psy_get_status(struct rt9471_chip *chip,
				 union power_supply_propval *val)
{
	...
	case RT9471_ICSTAT_VBUSRDY:
		//if(atomic_read(&chip->attach) && chg_en)//TODO
		if(chg_en)
...
static int rt9471_psy_get_ichg(struct rt9471_chip *data,
			       union power_supply_propval *val)
{
	u32 ichg;
	int ret;

	ret = __rt9471_get_ichg(data, &ichg);
	if (ret)
		return ret;

	val->intval = (unsigned int)ichg;
	return 0;
}

static int rt9471_psy_get_cv(struct rt9471_chip *data,
			     union power_supply_propval *val)
{
	u32 cv;
	int ret;

	ret = __rt9471_get_cv(data, &cv);
	if (ret)
		return ret;

	val->intval = (unsigned int)cv;
	return 0;
}

static int rt9471_psy_get_aicr(struct rt9471_chip *data,
			       union power_supply_propval *val)
{
	u32 aicr;
	int ret;

	ret = __rt9471_get_aicr(data, &aicr);
	if (ret)
		return ret;

	val->intval = (unsigned int)aicr;
	return 0;
}

static int rt9471_psy_get_mivr(struct rt9471_chip *data,
			       union power_supply_propval *val)
{
	u32 mivr;
	int ret;

	ret = __rt9471_get_mivr(data, &mivr);
	if (ret)
		return ret;

	val->intval = (unsigned int)mivr;
	return 0;
}

static int rt9471_psy_get_ieoc(struct rt9471_chip *data,
			       union power_supply_propval *val)
{
	u32 ieoc;
	int ret;

	ret = __rt9471_get_ieoc(data, &ieoc);
	if (ret)
		return ret;

	val->intval = (unsigned int)ieoc;
	return 0;
}

static int rt9471_psy_get_online(struct rt9471_chip *chip,
				 union power_supply_propval *val)
{
	mutex_lock(&chip->attach_lock);
	val->intval = atomic_read(&chip->attach);
	mutex_unlock(&chip->attach_lock);
...
static int rt9471_psy_get_property(struct power_supply *psy,
				   ...
				   union power_supply_propval *val)
{
	struct rt9471_chip *data = power_supply_get_drvdata(psy);
	int ret = 0;
	bool chg_en;
	
	ret = __rt9471_is_chg_enabled(data, &chg_en);

	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		ret = rt9471_psy_get_status(data, val);
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		ret = rt9471_psy_get_online(data, val);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		ret = rt9471_psy_get_ichg(data, val);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = RT9471_ICHG_MAXUA;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		ret = rt9471_psy_get_cv(data, val);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		val->intval = RT9471_CV_MAXUV;
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		ret = rt9471_psy_get_aicr(data, val);
		break;
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
		ret = rt9471_psy_get_mivr(data, val);
		break;
	...
	//	break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = rt9471_psy_get_ieoc(data, val);
		break;
...
static int rt9471_psy_enable_charging(struct rt9471_chip *chip, bool en)
{
	int ret;
...
static int rt9471_psy_set_ichg(struct rt9471_chip *chip, unsigned int ichg)
{
	int ret;

	ret = __rt9471_set_ichg(chip, ichg);

	return ret;
}

static int rt9471_psy_set_cv(struct rt9471_chip *chip, unsigned int cv)
{
	int ret;
	
	ret = __rt9471_set_cv(chip, cv);
	return ret;
}

static int rt9471_psy_set_aicr(struct rt9471_chip *chip, unsigned int aicr)
{
	int ret;

	ret = __rt9471_set_aicr(chip, aicr);

	return ret;
}

static int rt9471_psy_set_mivr(struct rt9471_chip *chip, unsigned int mivr)
{
	int ret;
	
	ret = __rt9471_set_mivr(chip, mivr);
	return ret;
}

static int rt9471_psy_set_ieoc(struct rt9471_chip *chip, unsigned int ieoc)
{
	int ret;

	ret = __rt9471_set_ieoc(chip, ieoc);

	return ret;
}

static int rt9471_bc12_preprocess(struct rt9471_chip *chip);
static int rt9471_psy_set_online(struct rt9471_chip *chip,
				const union power_supply_propval *val)
{
	...
	mutex_lock(&chip->attach_lock);
	if (pwr_rdy == !!atomic_read(&chip->attach)) {
		dev_err(chip->dev, "pwr_rdy is same(%d)\n", pwr_rdy);
		mutex_unlock(&chip->attach_lock);
		return 0;
	}
	...
	if (!chip->bc12_done)
		atomic_set(&chip->attach, attach);
	mutex_unlock(&chip->attach_lock);
	
	if (attach > ATTACH_TYPE_PD && chip->bc12_done)
		return 0;

	//if (!queue_work(chip->wq, &chip->bc12_work))
	//	dev_err(chip->dev, "bc12 work has already queued\n");
	mutex_lock(&chip->bc12_lock);
	atomic_set(&chip->vbus_gd, 1);
	ret = rt9471_bc12_preprocess(chip);
				   ...
				   const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		ret = rt9471_psy_enable_charging(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		ret = rt9471_psy_set_ichg(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		ret = rt9471_psy_set_cv(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		ret = rt9471_psy_set_aicr(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
		ret = rt9471_psy_set_mivr(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = rt9471_psy_set_ieoc(data, val->intval);
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		ret = rt9471_psy_set_online(data, val);
		break;
	case POWER_SUPPLY_PROP_TYPE:
...
static int rt9471_register_psy(struct rt9471_chip *data)
{
	struct power_supply_config cfg = {
		.drv_data		= data,
		.of_node		= data->dev->of_node,
		.supplied_to		= rt9471_psy_supplied_to,
		.num_supplicants	= ARRAY_SIZE(rt9471_psy_supplied_to),
	};
	struct power_supply_desc desc = {
		.name			= dev_name(data->dev),
		.type			= POWER_SUPPLY_TYPE_USB,
		.usb_types		= rt9471_chg_usb_types,
		.num_usb_types		= ARRAY_SIZE(rt9471_chg_usb_types),
		.properties		= rt9471_chg_properties,
		.num_properties		= ARRAY_SIZE(rt9471_chg_properties),
		.property_is_writeable	= rt9471_chg_prop_is_writeable,
		.get_property		= rt9471_psy_get_property,
		.set_property		= rt9471_psy_set_property,
	};
...
//regulator
static int rt9471_regulator_enable(struct regulator_dev *rdev)
{
...
static int rt9471_regulator_disable(struct regulator_dev *rdev)
{
	struct rt9471_chip *chip = rdev_get_drvdata(rdev);
	int ret = 0;
...
static int rt9471_regulator_is_enabled(struct regulator_dev *rdev)
{
    struct rt9471_chip *chip = rdev_get_drvdata(rdev);
	bool en = false;

	__rt9471_is_enabled_otg(chip, &en);

    return en;
}

static int rt9471_regulator_set_voltage_sel(struct regulator_dev *rdev,
                                      unsigned int selector)
{
    struct rt9471_chip *chip = rdev_get_drvdata(rdev);

	dev_info(chip->dev, "%s selector:%d\n", __func__, selector);

    return __rt9471_set_otgcv(chip, selector);
}

static int rt9471_regulator_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	dev_info(chip->dev, "%s val:%d\n", __func__, regval);
    return regval;
}

static int rt9471_regulator_set_current_limit(struct regulator_dev *rdev,
                                     int min_uA, int max_uA)
{
	...
	if (min_mA <= rt9471_otgcc[0])
		min_mA = rt9471_otgcc[0];
	else
		min_mA = rt9471_otgcc[1];
	dev_info(chip->dev, "%s min_uA:%d max_uA:%d min_mA:%d\n", __func__, min_uA, max_uA, min_mA);
    ret = __rt9471_set_otgcc(chip, min_mA);

    return ret;
}

static int rt9471_regulator_get_current_limit(struct regulator_dev *rdev)
{
    struct rt9471_chip *chip = rdev_get_drvdata(rdev);
	u32 mA;
	int ret;

	ret = __rt9471_get_otgcc(chip, &mA);
    if (ret < 0)
            return ret;
...
static const struct regulator_ops rt9471_otg_regulator_ops = {
	.enable = rt9471_regulator_enable,
	.disable = rt9471_regulator_disable,
	.is_enabled = rt9471_regulator_is_enabled,
	.list_voltage = regulator_list_voltage_linear,
	.set_voltage_sel = rt9471_regulator_set_voltage_sel,
	.get_voltage_sel = rt9471_regulator_get_voltage_sel,
	.set_current_limit = rt9471_regulator_set_current_limit,
	.get_current_limit = rt9471_regulator_get_current_limit,
};

static const struct regulator_desc rt9471_otg_desc = {
	.name = "usb-otg-vbus",
	...
	.uV_step = 150000,
	.n_voltages = 4,
	.curr_table = rt9471_otgcc,
	.n_current_limits = ARRAY_SIZE(rt9471_otgcc),
	.ops = &rt9471_otg_regulator_ops,
};

static int rt9471_register_regulator(struct rt9471_chip *chip)
{
	...
	chip->rdev = devm_regulator_register(chip->dev, &rt9471_otg_desc, &cfg);
	return IS_ERR(chip->rdev) ? PTR_ERR(chip->rdev) : 0;
}
...
static int rt9471_set_usbsw_state(struct rt9471_chip *chip, int usbsw)
{
	...
	dev_info(chip->dev, "%s usbsw=%d\n", __func__, usbsw);
	phy = phy_get(chip->dev, "usb2-phy");
	if (IS_ERR_OR_NULL(phy)) {
		dev_err(chip->dev, "failed to get usb2-phy\n");
		return -ENODEV;
	}
	ret = phy_set_mode_ext(phy, PHY_MODE_USB_DEVICE, mode);
	if (ret)
		dev_err(chip->dev, "failed to set phy ext mode\n");
...
wait:
	...
	ret = __rt9471_is_hz_enabled(chip, &chip->bc12_hz_state);
	if (chip->bc12_hz_state){
		dev_info(chip->dev, "%s HZ state:%d\n", __func__, chip->bc12_hz_state);
...
{
	int ret = 0;
	unsigned int attach = 0, inform_psy = true;
	...
	if (chip->bc12_hz_state){
		...
		chip->bc12_hz_state = false;
		dev_info(chip->dev, "%s rstore HZ state\n", __func__);
	}
	
	attach = atomic_read(&chip->attach);
	dev_info(chip->dev, "%s attach:%d bc12_done:%d\n", __func__, attach, chip->bc12_done);
	...
	case ATTACH_TYPE_TYPEC:
		...
		if (ret < 0) {
			dev_err(chip->dev, "failed to get port stat\n");
			inform_psy = false;
			goto out;
		}
		break;
	case ATTACH_TYPE_PD_SDP:
		chip->port = RT9471_PORTSTAT_SDP;
		break;
	case ATTACH_TYPE_PD_DCP:
		/* not to enable bc12 */
		chip->port = RT9471_PORTSTAT_DCP;
		chip->psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
		chip->psy_usb_type = POWER_SUPPLY_USB_TYPE_DCP;
		goto out;
	case ATTACH_TYPE_PD_NONSTD:
		chip->port = RT9471_PORTSTAT_NSDP;
		break;
	default:
		dev_err(chip->dev, "Invalid attach state\n");
		break;
	}
	...
	case RT9471_PORTSTAT_APPLE_10W:
		chip->psy_usb_type = POWER_SUPPLY_USB_TYPE_DCP;//APPLE_2_1A_CHARGER;
		break;
	case RT9471_PORTSTAT_APPLE_5W:
		chip->psy_usb_type = POWER_SUPPLY_USB_TYPE_DCP;//APPLE_1_0A_CHARGER;
		break;
	case RT9471_PORTSTAT_NSDP:
	default:
		inform_psy = false;
		chip->psy_usb_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;//NONSTANDARD_CHARGER;
		break;
	}
out:
	if (chip->psy_usb_type != POWER_SUPPLY_USB_TYPE_DCP)
		rt9471_enable_bc12(chip, false);
	if (inform_psy)
		power_supply_changed(chip->psy);
		//schedule_delayed_work(&chip->psy_dwork, 0);
	
	dev_info(chip->dev, "%s port = %s\n", __func__, rt9471_port_name[chip->port]);
...
//#ifdef CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT
	...
	rt9471_i2c_read_byte(chip, RT9471_REG_FUNCTION, &regval);
	dev_info(chip->dev, "%s REG_FUNCTION:%x\n", __func__, regval);
	rt9471_i2c_read_byte(chip, RT9471_REG_STAT0, &regval);
	dev_info(chip->dev, "%s REG_STATUS:%x\n", __func__, regval);
	__rt9471_is_hz_enabled(chip, &hz_state);
	dev_info(chip->dev, "%s __rt9471_is_hz_enabled:%x\n", __func__, hz_state);
	if (bc12_done) {
		mutex_lock(&chip->attach_lock);
		chip->bc12_done = true;
		mutex_unlock(&chip->attach_lock);
...
//#ifdef CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT
	...
	atomic_set(&chip->attach, ATTACH_TYPE_NONE);
	chip->attach_trig = ATTACH_TRIG_PWR_RDY;
	chip->port = RT9471_PORTSTAT_NOINFO;
	chip->bc12_hz_state = false;
	chip->psy_usb_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
//#endif /* CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT */
...
//#endif /* CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT */
	...
	if (ret) {
		dev_err(chip->dev, "Failed to register power supply\n");
		//goto out;
	}

	ret = rt9471_register_regulator(chip);
	if (ret) {
