...
static int32_t sd77428_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_TEMP:
        val->intval = chip->batt_info.batt_temp * 10;
        break;
...
int32_t sd77428rnt_get_battry_current(void)
{
    batt_data_t* pinfo = &g_chip_data->batt_info;
    return pinfo->batt_current;
}
EXPORT_SYMBOL(sd77428rnt_get_battry_current);
...
{   
    ...
    if (ret == 0)
    {
    batt_dbg("sd77428_rntpack vbat:%d, ibat:%05d, tbat:%d, rsoc:%03d, fcc:%d, dcap:%d, soh:%d, cycle:%d, rc:%d, ext_chg %d\n",
        pinfo->batt_voltage,
        pinfo->batt_current,
        pinfo->batt_temp,
        pinfo->batt_rsoc,
...
int32_t sd77428rnt_ic_init(struct sd77428_data *chip)
{
    ...
    schedule_delayed_work(&chip->sd77428_work, (5*HZ));                      //linux的队列任务管理，共享工作队列
