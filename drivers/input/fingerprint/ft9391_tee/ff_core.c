...
#define KOOBEE_READ_ID 1
#if KOOBEE_READ_ID
#define FF_VENDOR_INFO_LEN 8
static char g_vendor_info[FF_VENDOR_INFO_LEN] = {0};
...
static ssize_t chip_status_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    FF_LOGI("chip_id=0x%s\n", g_vendor_info);

    if (strstr(g_vendor_info, "93"))
        strncpy(chipid, "1", 2);
    else
        strncpy(chipid, "0", 2);

    return snprintf(buf, sizeof(chipid), "%s\n", chipid);
}
...
static int ff_set_spiclk(ff_context_t *ff_ctx, bool enable)
{
    ...
    if (ff_ctx && ff_ctx->b_spiclk && ff_ctx->spi) {
		...
		FF_LOGW("b_spiclk = %d ff_ctx->spi = %p ", ff_ctx->b_spiclk, ff_ctx->spi);
	}
#endif
    FF_LOGV("'%s' leave.", __func__);
    return 0;
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_SET_VENDOR_INFO:
        if (copy_from_user(g_vendor_info, (void *)arg, FF_VENDOR_INFO_LEN)) {
            FF_LOGE("copy_from_user(..) failed.");
            ret = (-EFAULT);
            break;
        }
        FF_LOGI("set g_vendor_info:%s", g_vendor_info);
        break;
...
int ff_init_driver(void)
{
    ...
    if(ret) {
        FF_LOGE("focaltech_finger sysfs_create_file failed\n");
    }
#endif
    FF_LOGD("'%s' leave.", __func__);
    return 0;
