...
static int fsfp_spi_clock_enable(bool on)
{
    ...
    static int count;
    ms = spi_master_get_devdata(fsfp_ctl_dev->pdev->master);
...
static int fsfp_ctl_device_reset(bool output)
{
    ...
    if (fsfp_ctl_dev->rst_num == 0) {
        xprintk(KERN_ERR, "fsfp_ctl_dev->rst_num is not get.\n");
        return -1;
    }

    if (output) {
        gpio_direction_output(fsfp_ctl_dev->rst_num, 1);
    }
    else {
        gpio_direction_output(fsfp_ctl_dev->rst_num, 0);
    }
...
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
static int fsfp_ctl_device_power_by_gpio(bool on)
{
    int err = 0;
    xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __FUNCTION__);
#if SF_MTK_CPU
    ...
    if (on) {
        err = pinctrl_select_state(fsfp_pinctrl, fsfp_pin_states[SF_PIN_STATE_PWR_ON]);
        msleep(10);
    }
    else {
        err = pinctrl_select_state(fsfp_pinctrl, fsfp_pin_states[SF_PIN_STATE_PWR_OFF]);
    }

#else

    if (on) {
        err = gpio_direction_output(fsfp_ctl_dev->pwr_num, 1);
        msleep(10);
    }
    else {
        err = gpio_direction_output(fsfp_ctl_dev->pwr_num, 0);
    }
...
static int fsfp_ctl_device_power_by_regulator(bool on)
{
    ...
    err = regulator_set_load(fsfp_ctl_dev->vdd_reg, 100000);
...
static int fsfp_ctl_device_power(bool on)
{
    int err = 0;
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
    err = fsfp_ctl_device_power_by_gpio(on);
...
#if SF_MTK_CPU
    fsfp_pinctrl = pinctrl_get(&ctl_dev->pdev->dev);
