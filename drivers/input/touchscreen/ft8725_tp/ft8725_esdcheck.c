...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    ...
    ret = ft8725_read_reg(FTS_REG_ESD_SATURATE, &val);
    if ( ret < 0) {
...
static int fts_esdcheck_tp_reset(struct fts_ts_data *ts_data)
{
    ...
    ft8725_reset_proc(ts_data, false, 200);
    ft8725_release_all_finger();
    ft8725_tp_state_recovery(ts_data);
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (ret < 0) {
            ...
            if ((idh == chip_id) || (ft8725_check_cid(ts_data, idh) == 0)) {
                break;
            } else {
...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
    ...
    ret = ft8725_read(&reg_addr, 1, &reg_value, 1);
    if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    ret = ft8725_read_reg(reg_addr, &reg_value);
    if ( ret < 0 ) {
        fts_esdcheck_data.nack_cnt++;
...
/*****************************************************************************
*  Name: ft8725_esdcheck_proc_busy
...
void ft8725_esdcheck_resume(struct fts_ts_data *ts_data)
{
    if (ts_data->esd_support) {
        FTS_FUNC_ENTER();
        ft8725_esdcheck_switch(ts_data, ENABLE);
