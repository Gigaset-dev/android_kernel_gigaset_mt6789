...
{
    int ret = 0;
    struct spi_device *spi = ft8725_data->spi;
...
int ft8725_write_reg(u8 addr, u8 value)
{
    ...
    writebuf[0] = addr;
    writebuf[1] = value;
    return ft8725_write(writebuf, 2);
}
...
{
    return ft8725_read(&addr, 1, value, 1);
}
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    if (!ts_data) {
        FTS_ERROR("allocate memory for ft8725_data fail");
        return -ENOMEM;
    }
    ...
    ret = ft8725_ts_probe_entry(ts_data);
    if (ret) {
...
static void fts_ts_remove(struct spi_device *spi)
{
    ...
    FTS_FUNC_ENTER();
    if (ts_data) {
        ft8725_ts_remove_entry(ts_data);
        spi_set_drvdata(spi, NULL);
        kfree_safe(ts_data);
    }
    FTS_FUNC_EXIT();
...
static int __init ft8725_ts_spi_init(void)
{
    int ret = 0;
...
static void __exit ft8725_ts_spi_exit(void)
{
    spi_unregister_driver(&fts_ts_spi_driver);
}

module_init(ft8725_ts_spi_init);
module_exit(ft8725_ts_spi_exit);
