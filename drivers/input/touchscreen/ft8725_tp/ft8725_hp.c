...
static int fhp_ioctl_reset(struct fhp_core_data *fhp_data, unsigned long high)
{
    ...
    return ft8725_set_reset(fhp_data->ts_data, (int)high);
}
...
static long fhp_input_report(struct fhp_core_data *fhp_data, unsigned long arg)
{
    ...
    if ((report_buf[1] == 0xFF) && (report_buf[2] == 0xFF) && (report_buf[3] == 0xFF) && (report_buf[4] == 0xFF)) {
        ...
        ft8725_tp_state_recovery(fhp_data->ts_data);
        return 0;
    }

    ret = ft8725_input_report_buffer(fhp_data->ts_data, report_buf);
    if (ret < 0) {
...
int ft8725_fhp_irq_handler(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->suspended && ts_data->gesture_support) {
        ft8725_gesture_readdata(ts_data, NULL);
        return 0;
    }

    ret = ft8725_read(&cmd, 1, ts_data->touch_buf, ts_data->touch_size);
    if (ret < 0) {
