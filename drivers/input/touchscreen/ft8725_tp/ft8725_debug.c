...
static int fts_fwdbg_get_cfg(struct fts_fwdbg *dbg)
{
    ...
    cmd = FTS_REG_TPINFO;
    ret = ft8725_read(&cmd, 1, tp_info, MAX_SIZE_TP_INFO);
    if (ret < 0) {
        FTS_ERROR("read tp info failed");
        return ret;
    }
    cmd = FTS_REG_DBGCFG;
    ret = ft8725_read(&cmd, 1, dbg_cfg, MAX_SIZE_DBG_CFG);
    if (ret < 0) {
...
static int fts_fwdbg_enable(struct fts_fwdbg *dbg, int value)
{
    ...
    ret = ft8725_write_reg(FTS_REG_FW_DEBUG_EN, value);
    if (ret < 0) {
...
dbgen_err:
    ...
    ft8725_write_reg(FTS_REG_FW_DEBUG_EN, 0);
    return ret;
}
...
static int fts_fwdbg_disable(struct fts_fwdbg *dbg)
{
    ...
    dbg->ts_data->touch_size = dbg->touch_size_bak;
    ret = ft8725_write_reg(FTS_REG_FW_DEBUG_EN, 0);
    if (ret < 0) {
...
static void fts_logging_regfb(struct fts_fwdbg *dbg, u64 timestamp)
{
    ...
    ret = ft8725_read(&cmd, 1, dbg->regfb_val, dbg->cfg.regfb_len);
    if (ret < 0) {
...
static void fts_logging_regfa(struct fts_fwdbg *dbg, u64 timestamp)
{
    ...
    ret = ft8725_read(&cmd, 1, dbg->regfa_val, dbg->cfg.regfa_len);
    if (ret < 0) {
...
static ssize_t fts_fwdbg_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8725_read_reg(FTS_REG_FW_DEBUG_EN, &val);
