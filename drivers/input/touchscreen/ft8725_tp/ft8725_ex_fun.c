    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_SET_TEST_FLAG:
        ...
        if (writebuf[1] == 0) {
            if (!ts_data->suspended) ft8725_esdcheck_switch(ts_data, ENABLE);
        } else {
    ...
    case PROC_WRITE_REGISTER:
        ret = ft8725_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        if (ts_data->bus_type == BUS_TYPE_I2C) {
            ret = ft8725_write(writebuf + 1, writelen);
            if (ret < 0) {
    ...
    case PROC_SET_SLAVE_ADDR:
        if (ts_data->bus_type == BUS_TYPE_I2C) {
            ft8725_bus_configure(ts_data, &writebuf[1], buflen - 1);
        }
        break;
    ...
    case PROC_HW_RESET:
        if (buflen < PROC_BUF_SIZE) {
            ...
            if (strncmp(tmp, "focal_driver", 12) == 0) {
                ...
                ft8725_reset_proc(ts_data, false, 0);
            }
        }
        break;
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            ft8725_enter_test_environment(0);
        } else {
            ft8725_enter_test_environment(1);
        }
        break;
    ...
    case PROC_WRITE_DATA_DIRECT:
        writelen = buflen - 1;
        ret = ft8725_bus_transfer_direct(writebuf + 1, writelen, NULL, 0);
        if (ret < 0) {
    ...
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = ft8725_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        if (ts_data->bus_type == BUS_TYPE_SPI)
            ret = ft8725_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        else if (ts_data->bus_type == BUS_TYPE_I2C)
            ret = ft8725_read(NULL, 0, readbuf, num_read_chars);
    ...
    case PROC_READ_DATA_DIRECT:
        num_read_chars = buflen;
        ret = ft8725_bus_transfer_direct(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
int ft8725_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
...
void ft8725_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = ft8725_read_reg(FTS_REG_FW_VER, &fwver);
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = ft8725_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = ft8725_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = ft8725_write_reg(reg, val);
            } else {
                rw_op.res = ft8725_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8725_read_reg(FTS_REG_CHARGER_MODE_EN, &val);
    count += snprintf(buf + count, PAGE_SIZE, "charge stat:0x%02x\n", val);

    ft8725_read_reg(FTS_REG_INT_CNT, &val);
    count += snprintf(buf + count, PAGE_SIZE, "INT count:0x%02x\n", val);

    ft8725_read_reg(FTS_REG_FLOW_WORK_CNT, &val);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8725_read_reg(FTS_REG_FOD_MODE_EN, &val);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&ts_data->input_dev->mutex);
    if (FTS_SYSFS_ECHO_ON(buf)) {
        ft8725_fod_enable(ENABLE);
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        ft8725_fod_enable(DISABLE);
    }
    mutex_unlock(&ts_data->input_dev->mutex);
