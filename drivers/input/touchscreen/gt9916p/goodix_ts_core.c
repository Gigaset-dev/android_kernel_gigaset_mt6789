...
#if IS_ENABLED(CONFIG_DRM_PANEL)
/**
 * goodix_ts_fb_notifier_callback - mtk disp notifier callback
	...
	unsigned long value, void *v)
{
	...
	if (core_data && v) {
		...
		ts_info("goodix touch IC can not suspend or resume\n");
		return -ECANCELED;
	}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	ret = mtk_disp_notifier_register("GOODIX Touch", &cd->fb_notifier);
	if (ret)
