...
#define HX_PROC_TOUCH_FOLDER "hx_android_touch"
...
/* drv added by huangyu, wakeup gesture, start */
struct himax_ts_data *g_drv_common_node_data = NULL;
#if defined(HX_SMART_WAKEUP)
bool himax_is_gesture_wakeup_enabled(void)
{
    return g_drv_common_node_data->gesture_wakeup_enabled;
}
EXPORT_SYMBOL_GPL(himax_is_gesture_wakeup_enabled);
...
#endif
/* drv added by huangyu, wakeup gesture, end */

/* drv added by huangyu, glove mode, start */
static void drv_glove_mode_enable(unsigned char on)
{
	struct himax_ts_data *ts = hx_s_ts;
	I("%s set drv_glove_mode_enable = %d\n", __func__, on);
	if (1 == on) {
		ts->HSEN_enable = 1;
		g_drv_common_node_data->glove_mode_enable = true;
		I("%s set HSEN_enable enable\n", __func__);
	} else if (0 == on) {
		ts->HSEN_enable = 0;
		g_drv_common_node_data->glove_mode_enable = false;
		I("%s set HSEN_enable disenable\n", __func__);
	}
}
...
static ssize_t drv_glove_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    if (buf[0] == '1') {
		...
		I("enable glove mode, glove_mode_enable = %d\n", g_drv_common_node_data->glove_mode_enable);
    } else if (buf[0] == '0') {
		drv_glove_mode_enable(0);
		I("disable glove mode, glove_mode_enable = %d\n", g_drv_common_node_data->glove_mode_enable);
    }
...
int drv_glove_init(void)
{
	...
	I("%s kobject_create_and_add end\n", __func__);
    err = sysfs_create_link(drv_glove, &client->dev.kobj, "common_node"); //&g_drv_common_node_data->i2c->dev.kobj
    if (err) {
        E("sysfs_create_link fail\n");
        return -1;
	}

    if (sysfs_create_file(&client->dev.kobj, &dev_attr_state.attr)) { //&g_drv_common_node_data->i2c->dev.kobj
        return -1;
	}

	return 0;
}
/* drv added by huangyu, glove mode, end */
...
int himax_common_proc_init(void)
{
	...
	if (proc_self_test_file == NULL) {
		...
		/* drv added by huangyu, wakeup gesture, start */
	#if defined(HX_SMART_WAKEUP)
	g_drv_common_node_data = hx_s_ts;
	drv_gesture_init();
	drv_glove_init();
	#endif
	/* drv added by huangyu, wakeup gesture, end */	
	...
	if (touch_proc_dir == NULL) {
		I("%s: touch_proc_dir file create failed!\n", __func__);
	}
	...
	if (report_status_control_file == NULL) {
		I("%s: proc report control file create failed!\n", __func__);
