	...
	struct pt_pen *pen, u8 *xy_data)
{
	...
	for (abs = PT_PEN_X; abs < PT_PEN_NUM_ABS; abs++) {
		...
		pt_get_touch_field(dev, &pen->abs[abs],
			si->pen_abs[abs].size,
			si->pen_abs[abs].max,
			xy_data + si->pen_abs[abs].ofs,
			si->pen_abs[abs].bofs);

		/*
		 * The value of TILT takes 2 bytes and will be 0x0000xxxx
		 * after assigning to variable "abs" of type "int" . So the
		 * sign bit needs to be filled if it is negative.
		 */
		if (abs == PT_PEN_X_TILT || abs == PT_PEN_Y_TILT)
			pen->abs[abs] = sign_extend32(pen->abs[abs], 15);
...
static int pt_setup_input_device(struct device *dev)
{
	...
	for (i = PT_PEN_X; i < PT_PEN_NUM_ABS; i++) {
		...
		case HID_DG_INVERT:
			...
			break;
		case HID_DG_TILT_X:
			max_x = pend->si->pen_abs[i].logical_max;
			/*
			 * TILT is 16-bit signed value in our case, and TILT Min
			 * is negative.
			 */
			min =
			    sign_extend32(pend->si->pen_abs[i].logical_min, 15);
			input_set_abs_params(pend->input,
				ABS_TILT_X, min, max_x, 0, 0);
			break;
		case HID_DG_TILT_Y:
			max_x = pend->si->pen_abs[i].logical_max;
			/*
