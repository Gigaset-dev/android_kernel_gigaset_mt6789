...
*****************************************************************************/
//drv-modify by shenwenbin for starup upgrade TP firmnware with .i 20231211 start
#define FTS_FW_REQUEST_SUPPORT                      0
/* Example: focaltech_ts_fw_tianma.bin */
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focaltech_ts_fw_visionox_ft3681_Ver0x01_VID0x44_PID0x44_20230816_app"
//drv-modify by shenwenbin for starup upgrade TP firmnware with .i 20231211 end
...
//drv add by shenwenbin for lcd hardware info 20231214 start
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	    ret = fts_read_reg(FTS_REG_FW_VER, &tp_fw_ver);
	    if ((ret < 0) || (tp_fw_ver == 0xFF) || (tp_fw_ver == 0x00)){
	        strcpy(current_tp_info.chip, "FT3681,Firmware version:get tp fw version fail!\n");
        }else{
	        sprintf(current_tp_info.chip, "FT3681,Firmware version:0x%02x",tp_fw_ver);
        }
