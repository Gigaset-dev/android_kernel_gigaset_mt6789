...
#include "../../panel/include/panel-visionox-rm692e0-dphy-cmd-120hz.h"	//drv-Fixed the issue of entering aod and TP having touch-pengzhipeng-20230516-end
/*****************************************************************************
...
static int fts_fod_readdata(struct fts_ts_data *ts_data)
{
	...
	FTS_DEBUG("fod_val[1]: %x: doze=%d,fod_figer_state=%d",fod_val[1],panel_visionox_fod_is_enabled(),atomic_read(&ts_data->fod_figer_state));
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    if (!ts_data->suspended || drv_get_doze_state()) {
...
#endif
        FTS_DEBUG("notifier,event:%lu,blank:%d", event, blank_value);
