...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
	gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &ctp_fw_version, 1);
	sprintf(current_tp_info.chip,"GT911,fw_version:0x%02x", ctp_fw_version);
	sprintf(current_tp_info.id,"0x%04x",sensor_id);
	strcpy(current_tp_info.vendor,"goodix");
	sprintf(current_tp_info.more,"%d*%d",1600,2560);
 #endif
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK) || defined(CONFIG_FB)
...
static int gtp_disp_notifier_callback(struct notifier_block *noti,
				    unsigned long event, void *data)
{
	struct goodix_ts_data *ts = container_of(noti,
			struct goodix_ts_data, disp_notifier);
	int *blank = (int *)data;

	if (data && ts) {
			dev_info(&ts->client->dev, "%s IN", __func__);
			...
			dev_info(&ts->client->dev, "%s OUT", __func__);
			} else {
			dev_info(&ts->client->dev, "%s GTP touch IC can not suspend or resume", __func__);
			return -1;
			}
...
static int gtp_register_powermanager(struct goodix_ts_data *ts)
{
	...
	ts->disp_notifier.notifier_call = gtp_disp_notifier_callback;
	ret = mtk_disp_notifier_register("GTP_TOUCH", &ts->disp_notifier);
	if (ret)
		dev_err(&ts->client->dev,
			"Unable to register disp_notifier: %d\n", ret);
#elif defined(CONFIG_FB)
	INIT_WORK(&ts->fb_notify_work, fb_notify_resume_work);
