...
static int hyn_parse_dt(struct hyn_ts_data *ts_data)
{
    ...
    if(dev->of_node){
        ...
        if(of_property_read_bool(np,"vcc_i2c") && of_property_read_bool(np,"vdd_ana")){
            ...
            HYN_INFO("vcc_i2c or vdd_ana not config");
            dt->vdd_i2c = NULL;
            dt->vdd_ana = NULL;
        }
        ...
        if (!IS_ERR_OR_NULL(dt->pinctl)){
            dt->pin_active = pinctrl_lookup_state(dt->pinctl, "cst_ts_active");
            dt->pin_suspend= pinctrl_lookup_state(dt->pinctl, "cst_ts_suspend");
...
static int hyn_power_source_ctrl(int enable)
{
    int ret = 0;
    HYN_ENTER();
    if(IS_ERR_OR_NULL(hyn_data->plat_data.vdd_i2c) || IS_ERR_OR_NULL(hyn_data->plat_data.vdd_ana)){
        return ret;
    }
...
static void hyn_resum(struct device *dev)
{
    ...
    //}
	if(hyn_data->gesture_is_enable){
        hyn_irq_set(hyn_data,DISABLE);
        ret = disable_irq_wake(hyn_data->client->irq);
        ret |= irq_set_irq_type(hyn_data->client->irq,(IRQF_TRIGGER_FALLING | IRQF_ONESHOT)); 
        if(ret < 0){
...
static irqreturn_t hyn_irq_handler(int irq, void *data)
{
	...
	//HYN_INFO("%s hyn_data->work_mode = %d\n", __func__, hyn_data->work_mode);
...
{
	struct hyn_ts_data *hyn_data =container_of(self, struct hyn_ts_data, fb_notif);
	int *evdata = (int *)data;
	...
	if (hyn_data && data) {
		HYN_INFO("hyn_plat_notifier_callback IN");
		if (event == MTK_DISP_EARLY_EVENT_BLANK) {
			if (*evdata == MTK_DISP_BLANK_POWERDOWN) {
				HYN_INFO("Suspend by mtk disp notifier.");
			...
			if (*evdata == MTK_DISP_BLANK_UNBLANK) {
				HYN_INFO("Resume by mtk disp notifier.");
				queue_work(hyn_data->hyn_workqueue, &hyn_data->work_resume);
			}
		}
		HYN_INFO("hyn_plat_notifier_callback OUT");
	} else {
		HYN_INFO("hyn touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
