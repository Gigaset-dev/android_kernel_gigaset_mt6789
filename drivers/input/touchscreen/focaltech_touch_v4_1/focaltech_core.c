...
/* DRV modified by chenjiaxi, support CONFIG_DRM_MEDIATEK_V2, start */
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
#include "mtk_disp_notify.h"
#else
#include <drm/drm_panel.h>
#endif //CONFIG_DRM_MEDIATEK_V2
/* DRV modified by chenjiaxi, support CONFIG_DRM_MEDIATEK_V2, end */
#else
...
/* DRV added by chenjiaxi, hardware info, start */
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../misc/mediatek/prize/hardware_info/hardware_info.h"
#endif
/* DRV added by chenjiaxi, hardware info, end */
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        ...
        /* DRV modified by chenjiaxi, support power gpio control, start */
        #if FTS_POWER_SOURCE_GPIO_CONTROL_EN
			if (gpio_is_valid(ts_data->pdata->vdd_gpio)) {
				ret = gpio_direction_output(ts_data->pdata->vdd_gpio, 1);
				if (ret) {
					FTS_ERROR("[GPIO]set vdd gpio output 1 failed");
				}
			}
			if (gpio_is_valid(ts_data->pdata->vcc_i2c_gpio)) {
				ret = gpio_direction_output(ts_data->pdata->vcc_i2c_gpio, 1);
				if (ret) {
					FTS_ERROR("[GPIO]set vcc_i2c gpio output 1 failed");
				}
			}
        #endif
        /* DRV modified by chenjiaxi, support power gpio control, end */
        ...
        /* DRV modified by chenjiaxi, support power gpio control, start */
			...
			if (gpio_is_valid(ts_data->pdata->vdd_gpio)) {
				...
				if (ret) {
					FTS_ERROR("[GPIO]set vdd gpio output 0 failed");
				}
			}
			if (gpio_is_valid(ts_data->pdata->vcc_i2c_gpio)) {
				ret = gpio_direction_output(ts_data->pdata->vcc_i2c_gpio, 0);
				if (ret) {
					FTS_ERROR("[GPIO]set vcc_i2c gpio output 0 failed");
				}
			}
        #endif
        /* DRV modified by chenjiaxi, support power gpio control, end */
...
/* DRV modified by chenjiaxi, support power gpio control, start */
    ...
    if (gpio_is_valid(ts_data->pdata->vcc_i2c_gpio)) {
        ret = gpio_request(ts_data->pdata->vcc_i2c_gpio, "fts_vcc_i2c_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]vcc_i2c gpio request failed");
        }
    }
#endif
/* DRV modified by chenjiaxi, support power gpio control, end */
...
/* DRV modified by chenjiaxi, support power gpio control, start */
    ...
    if (gpio_is_valid(ts_data->pdata->vcc_i2c_gpio))
        gpio_free(ts_data->pdata->vcc_i2c_gpio);
#endif
/* DRV modified by chenjiaxi, support power gpio control, end */
    return 0;
}
...
/* DRV added by chenjiaxi, hardware info, start */
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_tp_info.more,"%d*%d", pdata->x_max, pdata->y_max);
#endif
/* DRV added by chenjiaxi, hardware info, end */
...
/* DRV modified by chenjiaxi, support power gpio control, start */
    ...
    pdata->vcc_i2c_gpio = of_get_named_gpio_flags(np, "focaltech,vcc_i2c-gpio",
                        0, &pdata->vcc_i2c_gpio_flags);
    if (pdata->vcc_i2c_gpio < 0)
        FTS_ERROR("Unable to get vcc_i2c_gpio");
#endif
/* DRV modified by chenjiaxi, support power gpio control, end */
...
/* DRV modified by chenjiaxi, support CONFIG_DRM_MEDIATEK_V2, start */
    ...
    ret = mtk_disp_notifier_register("Touch", &ts_data->fb_notif);
    if (ret) {
