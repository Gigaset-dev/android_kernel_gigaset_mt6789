 ...
 *
 * $Revision: 108740 $
 * $Date: 2022-11-21 10:30:18 +0800 (周一, 21 11月 2022) $
 *
...
/* DRV added by chenjiaxi, hardware_info, start */
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.id, "fw:0x%02x,pid:0x%04x", ts->fw_ver,ts->nvt_pid);
	strcpy(current_tp_info.vendor, "Novatek");
	sprintf(current_tp_info.more, "%d*%d", ts->abs_y_max,ts->abs_x_max);
#endif
/* DRV added by chenjiaxi, hardware_info, end */
	ret = 0;
out:
