...
//drv-mod by shenwenbin for miniscreen TP hardware info 20240722 start
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
#if IS_ENABLED(CONFIG_DRV_MINISCREEN_TP)
#include "../../../misc/mediatek/prize/hardware_info/hardware_info.h"
extern struct hardware_info miniscreen_tp_info;
#endif
#endif
//drv-mod by shenwenbin for miniscreen TP hardware info 20240722 end
...
struct hynitron_fw_array hynitron_fw_grp[20] = {
	...
	{ "capacitive_hynitron_cst3xx_update",   cst3xx_fw,   0x2117,0x11, CST328, (sizeof(cst3xx_fw))},//0x444e for N1
	...
	{ "capacitive_hynitron_cst8xx_update",   cst8xx_fw,   0x0501,0x01, CST836, (sizeof(cst8xx_fw))},
  	...
  	{ "capacitive_hynitron_cst92xx_update",  cst92xx_fw,  0x554B,0x01, CST9217, (sizeof(cst92xx_fw))},
...
int hyn_auto_read_file(char *file_name, u8 **file_buf)
{
	...
	*file_buf = vmalloc(file_len);
	...
	pos = 0;
	//ret = vfs_read(filp, *file_buf, file_len, &pos); //drv modify by wangwei1 for GKI check error 20230526
	if (ret < 0)
...
int cst3xx_firmware_info(struct i2c_client * client)
{
	...
	if((hyn_ts_data->chip_ic_checkcode&0xffff0000)!=0xCBCB0000){
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
#if IS_ENABLED(CONFIG_DRV_MINISCREEN_TP)
	sprintf(miniscreen_tp_info.chip, "CST9217,FW_VER:0x%x", hyn_ts_data->chip_ic_fw_version);
	sprintf(miniscreen_tp_info.id,"0x%04x",hyn_ts_data->chip_ic_project_id);
	sprintf(miniscreen_tp_info.more, "%d*%d",228,460);
	strcpy(miniscreen_tp_info.vendor,"hynitron");
#endif
#endif
//drv-mod by shenwenbin for miniscreen TP hardware info 20240722 end
