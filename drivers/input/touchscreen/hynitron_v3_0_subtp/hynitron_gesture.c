...
int hyn_create_gesture_sysfs(struct i2c_client *client)
{
    ...
    ret = sysfs_create_group(&hyn_ts_data->dev->kobj, &hyn_gesture_group);
    if ( ret != 0){
        HYN_ERROR( "[GESTURE]hyn_gesture_mode_group(sysfs) create failed!");
        sysfs_remove_group(&hyn_ts_data->dev->kobj, &hyn_gesture_group);
        return ret;
    }
	HYN_FUNC_EXIT();
    return 0;
...
void hyn_gesture_recovery(struct i2c_client *client)
{
    ...
    if (hyn_gesture_data.mode && hyn_gesture_data.active)
    {
		if(hyn_ts_data->config_chip_product_line==HYN_CHIP_PRODUCT_LINE_MUT_CAP){
			u8 buf[2];
			buf[0]=HYN_REG_MUT_GESTRUE_MODE>>8;
			buf[1]=HYN_REG_MUT_GESTRUE_MODE&0xff;
			cst3xx_i2c_write(client, buf, 2);
		}else{
...
int hyn_gesture_resume(void)
{
	...
	if(hyn_ts_data->config_chip_product_line==HYN_CHIP_PRODUCT_LINE_MUT_CAP){
		u8 buf[2];
		buf[0]=HYN_REG_MUT_NORMAL_MODE>>8;
		buf[1]=HYN_REG_MUT_NORMAL_MODE&0xff;
		cst3xx_i2c_write(hyn_ts_data->client, buf, 2);
