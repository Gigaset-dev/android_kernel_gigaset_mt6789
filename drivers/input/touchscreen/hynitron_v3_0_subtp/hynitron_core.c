...
//drv-mod by shenwenbin for customize suspend/resume 20240715 start
/*
...
#endif
*/
//drv-mod by shenwenbin for customize suspend/resume 20240715 end
...
*****************************************************************************/
//drv-mod by shenwenbin for adaptive miniscreen  app 20240722 start
#define HYN_DRIVER_NAME                     "gc9a01_touch"
//drv-mod by shenwenbin for adaptive miniscreen  app 20240722 end

/*****************************************************************************
* Static variables
*****************************************************************************/

static int hynitron_ts_is_probe_ok = false;
...
#if IS_ENABLED(CONFIG_PM) && HYN_PATCH_COMERR_PM
	int ret = 0;
	
	if ((hyn_ts_data->suspended) && (hyn_ts_data->pm_suspend)) {
			ret = wait_for_completion_timeout(
					  &hyn_ts_data->pm_completion,
					  msecs_to_jiffies(HYN_TIMEOUT_COMERR_PM));
		if (!ret) {
			HYN_ERROR("Bus don't resume from pm(deep),timeout,skip irq");
			return IRQ_HANDLED;
		}
	}
#endif
...
static int hyn_irq_registration(struct i2c_client *client)
{
    ...
    if (node)
    {
       ...
       // HYN_INFO("IRQs are inconsistent, please check <interrupts> & <hynitron,irq-gpio> in DTS");
        ...
        //hyn_ts_data->pdata->irq_gpio_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;
...
static void hyn_suspend(struct hynitron_ts_data *ts)
{
    ...
    if (hyn_ts_data->suspended) {
        HYN_DEBUG("Already in suspend state");
        return;
    }
    
    hyn_ts_data->suspended = true;
#if HYN_PSENSOR_EN
...
static void hyn_resume(struct hynitron_ts_data *ts)
{
    ...
    if (!hyn_ts_data->suspended) {
        HYN_DEBUG("Already in resume state");
        return;
    }

    hyn_ts_data->suspended = false;
...
static int hyn_plat_notifier_callback(struct notifier_block *self,
                                 unsigned long event, void *data)
{
	struct hynitron_ts_data *hynitron_data =container_of(self, struct hynitron_ts_data, fb_notif);
	int *evdata = (int *)data;
	...
	if (hynitron_data && data) {
		printk("hyn_plat_notifier_callback IN");
		if (event == MTK_DISP_EARLY_EVENT_BLANK) {
			if (*evdata == MTK_DISP_BLANK_POWERDOWN) {
				HYN_DEBUG("Suspend by mtk disp notifier.");
				cancel_work_sync(&hynitron_data->resume_work);
				hyn_suspend(hynitron_data);
			}
		} else if (event == MTK_DISP_EVENT_BLANK) {
			if (*evdata == MTK_DISP_BLANK_UNBLANK) {
				HYN_DEBUG("Resume by mtk disp notifier.");
				if (!work_pending (&hynitron_data->resume_work)){
					schedule_work (&hynitron_data->resume_work);
				}
			}
		}
		printk("hyn_plat_notifier_callback OUT");
	} else {
		printk("hyn touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
...
{
	if (hynitron_ts_is_probe_ok) {
		if(value){
			HYN_DEBUG("gc9a01_tp_power_ctrl HYN resume.");
			if (!work_pending (&hyn_ts_data->resume_work)){
			schedule_work (&hyn_ts_data->resume_work);
			}
		}else{
			HYN_DEBUG("gc9a01_tp_power_ctrl HYN suspend.");
			cancel_work_sync (&hyn_ts_data->resume_work);
			hyn_suspend(hyn_ts_data);
		}
	} else {
...
void gc9a01_tp_gesture_ctrl(int value)
{
	int ret = 0;
...
EXPORT_SYMBOL(gc9a01_tp_gesture_ctrl);
...
static int hyn_pm_suspend(struct device *dev)
{
    ...
    //hyn_suspend(hyn_ts_data);//drv add by wangwei1 not use pm wakeup 20230808 end
	return 0;
}
static int hyn_pm_resume(struct device *dev)
{
	...
	//hyn_resume(hyn_ts_data);//drv add by wangwei1 not use pm wakeup 20230808 end
	return 0;
...
{
    if(hyn_ts_data){
	    HYN_INFO("system enters into pm_suspend");
	    hyn_ts_data->pm_suspend = true;
	    reinit_completion(&hyn_ts_data->pm_completion);
    }
    return 0;
}
...
{
    if(hyn_ts_data){
	    HYN_INFO("system resumes from pm_suspend");
	    hyn_ts_data->pm_suspend = false;
	    complete(&hyn_ts_data->pm_completion);
    }
    return 0;
}

static const struct dev_pm_ops hyn_dev_pm_ops = {
...
/*
#if defined(CONFIG_FB)
HYN_ERROR("Touch Probe : hyn_fb_notifier_callback");
		ts_data->fb_notif.notifier_call = hyn_fb_notifier_callback;
		fb_register_client(&ts_data->fb_notif);
#elif defined(CONFIG_DRM)
HYN_ERROR("Touch Probe : hyn_plat_notifier_callback");
		ts_data->fb_notif.notifier_call = hyn_plat_notifier_callback;
        ret = mtk_disp_notifier_register("hynitron Touch", &ts_data->fb_notif);
        if (ret)
             HYN_ERROR("[DRM]drm_panel_notifier_register fail: %d\n", ret);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
//drv-mod by shenwenbin for customize suspend/resume 20240715 end
    ...
    hyn_ts_data->suspended = false;
	hynitron_ts_is_probe_ok = true;	//drv-mod by shenwenbin for customize suspend/resume 20240715
