...
static const struct proc_ops proc_tool_debug_fops = {
	.proc_read	    = hynitron_proc_read_foobar,
	.proc_write		= hynitron_proc_write_foobar, 	
};
...
int hyn_create_sysfs(struct i2c_client *client)
{
	...
	//	return  -EIO;
    //}
	err = sysfs_create_group(&hyn_ts_data->dev->kobj, &hyn_attribute_group);
	if (0 != err) {
		HYN_INFO("%s() - ERROR: sysfs_create_group() failed.\n",__func__);
		sysfs_remove_group(&hyn_ts_data->dev->kobj, &hyn_attribute_group);
		return -EIO;
	} else {
